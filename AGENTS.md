...
- Прод-бокс в `~/.ssh/config` — алиас `sarafan`; корень деплоя и env-файл приложения смотри в приватной памятке, в отслеживаемых файлах их нет ([«Что не публикуется»](#что-не-публикуется-секреты-и-инфра-детали)).
...
- **Не сочиняй текст внутри командной строки — вообще** (заказ владельца 2026-08-25, письмо
  ...
  **Почему:** текст в команде проходит через кавычки аргумента инструмента → парсер
  ...
  доэкранируй. Отказы этого класса: `unexpected EOF` на backticks (19.08), subject коммита
  стал `@` (G190 мозга), кириллица в аргументе `ssh` роняет соединение (G235).
