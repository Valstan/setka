...
- 🟢 **Хвост дистиллятора — перевести на v3-паттерн перед запуском:** заготовка промпта дистиллятора
  (`docs/ops/hitl-classifier-routine.md` §«Петля обучения») всё ещё v2-стиля (ключ в тексте + эссе) —
  отобьётся так же. Добавить в `scripts/classifier_routine.py` подкоманды `corrections`/`propose`
  (по образцу `fetch`/`submit`) и короткий промпт без ключа.
...
- ⏳ **Хвост А — вторая облачная рутина «дистилляция»:** owner создаёт в UI (как первую, заготовка промпта
  в `docs/ops/hitl-classifier-routine.md` §«Петля обучения»). **Владельцу нужно подробное объяснение:
  зачем она, как настроить и включить** — разобрать в следующей сессии перед запуском. Панель и ручной ввод
  правил работают и без неё.
