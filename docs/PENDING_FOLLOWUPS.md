...
`⏱ 2026-08-19 · snooze 0 · fresh · построено, ждёт деплоя`
...
- ⏳ **Не задеплоено** — нужен `/reliz` (правки не схемные, миграции нет).
- ⏳ **Живьём не проверено** — группировка и групповые кнопки собраны и покрыты
  тестами, но в браузере владельцем не смотрены.
- 🟡 **Похожесть только дословная.** На суточной очереди дубль-группы дают ~13%
  экономии нажатий (62 группы, 296 постов из 1742). Нечёткая похожесть («одна
  новость, пересказанная разными пабликами») отложена: на завале за 40 дней
  дубли давали 49%, на сутках — вчетверо меньше, и цена реализации выше выигрыша,
  пока блоки «вердикт × тема» не обкатаны.
