...
- **`logs/app.log` не пишется с 2026-05-22** — обнаружено 2026-05-25 при диагностике инцидента с discovery для `tuzha`. Файл `/home/valstan/SETKA/logs/app.log` — 23KB, последняя запись `2026-05-22 14:55`. Все логи FastAPI сейчас идут только в journalctl. Видимо `main.py` lifespan `FileHandler` сломался (rotation? permission? путь?) — нужно проверить `os.getenv("LOG_PATH")` и whether StreamHandler-fallback сработал тихо. Симптом не блокирующий, но усложняет grep по конкретным endpoint'ам / ошибкам.
