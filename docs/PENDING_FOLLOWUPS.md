...
- 🟢 **Позже (не срочно):** вычистить `VK_TOKEN_*` из `/etc/setka/setka.env`
  ...
  `web/api/publisher.py` ручная страница — на `create_with_policy`.
...
- 🟢 **Старый хвост облачной рутины:** (1) отключить локальную задачу `setka-hitl-classifier`
  ...
  планировщика уже НЕТ** (осталась осиротевшая папка `~/.claude/scheduled-tasks/setka-hitl-classifier/`
  со старым SKILL.md, где ingest-ключ открытым текстом — стоит удалить руками); проверить вечернюю
  машину. (2) снять коннектор Gamma с рутины (лишний контекст).
