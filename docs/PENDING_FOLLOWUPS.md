...
- ⏳ **Хвост А — вторая облачная рутина «дистилляция»:** код и док готовы (v3, 2026-07-10) — owner
  создаёт в UI по шагам `docs/ops/hitl-classifier-routine.md` §«Настройка второй рутины» (repo +
  env-ключ + Network Custom + промпт-заготовка). Объяснение «зачем и когда включать» — там же;
  запускать осмысленно при накоплении коррекций (≥ ~10). Панель и ручной ввод правил работают и без неё.
...
- 🟡 `⏱ 2026-07-07 · snooze 0 · fresh` **`scripts/smoke_test.py` не проходит auth к diagnostics-эндпоинту** →
  `HTTP 401 Unauthorized` при POST `/api/regions/{code}/diagnostics` (роут `web/api/regions.py:532`,
  закрыт session-гейтом `AuthGateMiddleware`; в smoke_test.py нет механизма токена/сессии, файл не менялся
  с #275). Гейт деплоя (dry-run пайплайна) фактически сломан — на релизе 2026-07-07 шаг 8.5 упал, но пайплайн
  жив (worker active, импорт ОК). Варианты: (а) diagnostics-роуты в PUBLIC_PREFIXES с X-API-Key (как
  `/api/classifier` ingest); (б) научить smoke_test.py аутентифицироваться; (в) внутренний seam без гейта.
  Заведена фоновая задача.
