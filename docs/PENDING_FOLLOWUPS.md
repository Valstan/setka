...
### 🌍 Экспансия на всю Кировскую область — скелет 27 районов готов, 3 группы созданы (заказ владельца 2026-07-18)
...
- ✅ **Батч №1 создан 2026-07-21 из-под VALSTAN:** Яранск (`yaransk_info43`, id 240386781),
  ...
  подписана и назначена редактором во всех трёх.
...
- ⏳ **Текущий батч: активация после выката multi-token каскада** — `UPDATE regions SET vk_group_id=-<id>, is_active=true`;
...
- ⏳ **Устранение единой точки отказа community-токенов (начато 2026-07-21):** ВСЕ старые community-токены
  ...
  user-токене. Multi-token каскад `COMM_<id>` → `COMM_<id>_MAMA` → user-токены построен
  и покрыт тестами; для первого батча выпущены 3 ключа VALSTAN. Остаток: выкатить код,
  выпустить/занести резервные ключи МАМЫ и поэтапно перевыпустить пары для старых групп.
