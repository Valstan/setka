...
- 🟡 **Пустые `reasons_json` при score из унаследованного `AdvertisementFilter`** — если пост набрал балл только базовым фильтром дайджеста (его порог отклонения 4 > порога кабинета 3) и без предложка-сигналов, `classify` помечает рекламу, но `reasons` пуст (в карточке не видно «почему»). Прокинуть внутренние причины/score базового фильтра в `reasons` при `score >= SCORE_THRESHOLD` (`modules/ad_cabinet/classifier.py`). Замечено на заявке #1 (pizhanka, score 3, reasons `[]`).
- 🟢 **`can_message` не пречекается в scanner** — определяется на `/send` (`isMessagesFromGroupAllowed`); UI показывает одну primary-кнопку и фолбэк на личный диалог по ответу. Пречек в скане — опц. оптимизация (минус VK-вызовы на заявку).
- 🟢 **Фаза 2 (остаток)** — наборы офферных картинок по регионам, bulk-действия в инбоксе, авто-send где `is_allowed=1` (под контролем оператора). Базовая библиотека картинок/текстов + выбор при сборке письма уже сделаны (см. выше).
- 🟢 **Фаза 3 (CRM)** — `ad_clients`/`ad_payments`/`ad_publications` (ключ `author_vk_id`), связь заявка→клиент, стадии сделки, учёт оплат и публикаций.
...
- 🟢 **Видео >50 MB / только-player VK-ролики не уходят в TG** (best-effort): `resolve_media` берёт только прямые `*.mp4` (Telegram не грузит embed/player по URL), оверсайз TG отклоняет → `degraded`, пост уходит без этого видео. Если понадобится — скачивать видео и слать файлом (`sendVideo` multipart) вместо URL. Не срочно.
...
- 🟢 **UI per-community Telegram-таргет** — сейчас `communities.telegram_channel/telegram_bot` задаются миграцией/SQL. Хочется поле в `/communities` (модалка) для добавления новых зеркал без миграции.
...
- 🟢 **kirov_obl `selhoz` — точечно добрать ВятГАТУ** (Вятский ГАТУ, агроуниверситет): в 2 прохода `/discover_communities` не всплыл чистым кандидатом (только принт-точка в нём). selhoz сейчас 5 (Минсельхоз, Агрофирмы КМК, сельхозтехникум, Управление ветеринарии, Россельхознадзор) — флагман-вуз был бы сильным добавлением. Резолвить хэндл напрямую (`groups.getById screen_name`).
...
- 🟢 **`tatarstan_obl` — добрать `promyshlennost`** (опц.): тема осталась пустой при засеве пула (чистого офиц. источника Татнефть/КАМАЗ корп. в проходе discovery не всплыло). Точечно резолвить хэндлы через `groups.getById screen_name` и засеять `seed_region_communities.py`. Не блокер — остальные 11 тем публикуют.
