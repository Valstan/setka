...
- ⏳ **Valstan заблокирован VK до 2026-05-28T06:59:03** — `vk_tokens.disabled_until` уже выставлен через `POST /api/tokens/VALSTAN/disable?hours=24`. После истечения SQL-фильтр `disabled_until > now()` перестанет его прятать автоматически. До восстановления: парсинг через Vita (auto, см. hot-fix [PR #72](https://github.com/Valstan/setka/pull/72)); wall.post районных дайджестов — через `COMM_<vk_id>` community-токены районов (все 13 в БД); wall.post в kirov_obl — недоступен (community-токена нет); wall.repost (copy_setka) — недоступен (VK API не принимает community-token).
- ⏳ **kirov_obl публиковать не сможет** до восстановления VALSTAN ИЛИ до создания community-token для группы `vk.com/kirovskaya_info` (vk_group_id=-168170001). Решение: в админке группы выписать community-token со scope `wall`, добавить в БД как `COMM_168170001`. После — TokenPolicy.pick(COMMUNITY_WRITE) подхватит его автоматически. Пока — все 6 beat-слотов `postopus-kirov-oblast-*` собирают дайджест корректно (12 районов, 55 кандидатов, фильтрация работает — подтверждено smoke 11:52 UTC), но финальный `wall.post` падает с `no publish-token available`.
...
- 🟡 **wall.repost при blocked Valstan** — copy_setka вернёт «no publish-token available». Альтернатива на будущее: добавить второго user-токена в `VK_PUBLISH_TOKEN_NAMES` (например OLGA) — TokenPolicy сама подхватит как fallback. Либо переписать copy_setka на копирование контента через wall.post вместо wall.repost.
