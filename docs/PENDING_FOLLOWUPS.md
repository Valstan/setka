...
**MVP закрыт 2026-05-22** (см. `DEV_HISTORY.md`):
- ✅ Миграция 011: `community_candidates` + `regions.vk_city_id/center_city` + `communities.health_*` + composite UNIQUE(region_id, vk_id).
- ✅ Backend: `modules/discovery/{vk_search,ai_categorizer}.py`, `tasks/discovery_tasks.py`.
...
- ✅ +60 тестов, 330/330 зелёных.

**Остаётся (вторая итерация, следующая сессия):**

- ⏳ **`recheck_existing_communities()`** — Celery beat weekly:
  - Для каждого `Community.is_active=True` — `wall.get(count=1)` → обновить `last_post_at`.
  - Нет постов > `region.config['dormant_days']` (default 60) → `health_status='dormant'`.
  - AI-check последних 10 постов → если категория сместилась → `health_status='changed_category'`, `suggested_category=<new>`.
  - VK errors 15/100/203 → `health_status='dead'`.
  - Telegram-alert по итогам: «по региону mi: 3 новых кандидата, 1 dead, 2 changed_category».
- ⏳ Beat entry: `discovery-recheck-weekly` (Mon 04:00). Опционально `discovery-rediscover-monthly` для повторного поиска.
