...
- ⏳ **A — реклама во входящих ЛС + диалог из кабинета. Бэкенд готов (2026-06-04, ветка `feat/ad-cabinet-block-a-inbound-dm`), UI остаётся.** Реклама теперь ловится не только в предложке, но и во входящих ЛС сообществ. Сделано: миграция 026 (`ad_requests.origin` `suggested`/`inbound_dm`, `vk_post_id` nullable, `last_message_id`, частичный уникальный индекс дедупа по `(community_vk_id, peer_id) WHERE origin='inbound_dm'`); `VKDialogsChecker` (`messages.getConversations`, нормализует последнее входящее сообщение в post-совместимый формат); `modules/ad_cabinet/dm_scanner.py` (`run_dm_scan` + `scan_region_dialogs`, тот же `classifier.classify`); celery-таска `scan_inbound_dm_ads` + beat `scan-inbound-dm-ads` (X:05/35, 8-22); `VKClient.get_message_history` (seam под тред-вью); `origin`-фильтр в `GET /api/ad-cabinet/requests`. Для inbound-ЛС `can_message` сразу `True` (автор написал первым → ответ в диалог разрешён), поэтому существующий `/send` отвечает из кабинета без доработок. +14 тестов. **Деплой:** миграция 026 + restart web/worker/**beat** (новая beat-задача). ⏳ _Остаток (PR 2, UI):_ вкладки/фильтр «предложка | личка» в `/ad-cabinet`, показ `dialog_url`, тред-вью истории через `get_message_history`.
