...
- 🟡 `⏱ 2026-07-29 · snooze 0 · fresh` **Приёмочный критерий «показать красный прогон» не выполнен.**
  ...
  Первый прогон сканера состоится на PR этой ветки: до него неизвестно ни то, чист ли репозиторий,
  ни то, сколько ложных совпадений даст дефолтный набор правил на живом коде (ожидаются фикстуры и
  примеры в доках; под них заведён `allowlist` в `.gitleaks.toml`). Оба вопроса закрываются
  результатом того прогона, красный прогон на синтетическом секрете — отдельным шагом.
