...
### ~~🔴 Вход через ВКонтакте сломан во всей экосистеме: `RADAR_ID_VK_APP_ID` указывает на несуществующее приложение~~ ЗАКРЫТО 2026-07-26 — ложная тревога, приложение живо

`⏱ 2026-07-25 · закрыто 2026-07-26`

**Развязка 2026-07-26:** приложение 54666252 «единый вход вМалмыже» СУЩЕСТВУЕТ — оно
заведено в **новом кабинете VK ID** (`id.vk.ru/about/business/go`, аккаунт 100394),
а проверка 2026-07-25 искала его в старом `dev.vk.ru`, где VK ID-приложения не
отображаются. Конфиг корректен: платформа Web, включено; базовые домены
`вход.вмалмыже.рф` + `сарафан.вмалмыже.рф`; доверенные redirect
`https://<punycode вход|сарафан>/auth/vk/callback`. Live-проверено в браузере:
`/auth/vk/login` → id.vk.ru открывает диалог «Выбрать аккаунт» без ошибки про
базовый домен. **Урок:** VK ID-приложения искать в `id.vk.ru/about/business/go`,
не в `dev.vk.ru` (там только Мини-приложения/игры).

Проверено вживую 2026-07-25 (тот самый пункт «владельцу физически проверить
`вход.вмалмыже.рф/auth/vk/login`», который висел непроверенным): **вход через ВК не
работает нигде** — VK ID отвечает «в настройках ресурса не указан данный базовый домен»
и для `…myjino.ru`, и для `вход.вмалмыже.рф`.

Причина глубже, чем незаполненный домен:

1. Прод настроен на `RADAR_ID_VK_APP_ID=54666252`.
2. В кабинете разработчика **под VALSTAN этого приложения нет**:
   `dev.vk.ru/ru/admin/app-settings/54666252/info` → «Страница не найдена».
3. Единственное приложение аккаунта — `setka`, id **54542311**, тип **Мини-приложение**,
   состояние **«Откл»** («Приложение недоступно по URL»). У него вообще нет полей
   «базовый домен» / «доверенный redirect URI» — ни в «Размещении», ни в
   «Дополнительных» (там только консоль Eruda). Мини-приложение — не тот тип.

Значит починка — не «дописать домен в настройках», а сначала выяснить, **где заведено
54666252** (другой VK-аккаунт? удалено?), либо завести подключение заново:
`dev.vk.ru` → Приложения → **«Подключение VK ID»** с базовым доменом
`вход.вмалмыже.рф` и доверенным redirect URI
`https://вход.вмалмыже.рф/auth/vk/callback`, после чего перевести
`RADAR_ID_VK_APP_ID` (+ секрет) на новое подключение и перезапустить web.

Пока не починено, единый вход «Сервисы Малмыжа» через ВК недоступен для всех сервисов
экосистемы; вход по логину/паролю работает.

**Кодовая сторона единого входа ГОТОВА и ЗАДЕПЛОЕНА 2026-07-26 (PR #383, прод `f95f21e`):**
неавторизованный пользователь любого сервисного поддомена `*.вмалмыже.рф` уезжает на
центральный `вход.вмалмыже.рф/login?next=https://<родной-хост>/...`, выбирает способ входа
и возвращается на родной домен авторизованным (кука на всю зону; анти-open-redirect по
`SESSION_COOKIE_DOMAIN`; брендинг страницы — по хосту родного сервиса). Проверено live:
302 с радар-хоста на центральный логин, issuer — локальный `/login`, чужой хост срезается.
~~Остался ход владельца: создать приложение~~ — не потребовалось, см. развязку выше
(приложение существовало в id.vk.ru-кабинете). Осталась только живая проверка входа
владельцем (кликнуть свой аккаунт в диалоге VK).
