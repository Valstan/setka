...
- ⏳ **Ф1.2 — enforcement квот архива** (операционный риск №1 по brain: tiny-бокс + «вечный» архив →
  переполнение диска убьёт поллер/Postgres). **Построено 2026-06-13** (ветка `feat/radar-quota-enforcement`):
...
- 🟢 **Ф1.3 — PNG-иконки PWA** (quick win) — проверить, не закрыто ли уже Ф0.4 (`scripts/generate_radar_icons.py`,
  `icon-192/512.png` коммичены 2026-06-12); если да — отчитаться brain, что п.3 уже сделан.
- ⏸ **Ф1.4 — фоновое скачивание TG-медиа** `parked` до probe: brain требует СНАЧАЛА probe (#020) скачивания
  5–10 TG-медиа **прямо с бокса** (не через CF) — бокс datacenter, G56-тарпит может бить и его. Probe-результат
  письмом, не уходить в стройку обходных путей. Владелец: медиа в сохранёнках «не критично».
