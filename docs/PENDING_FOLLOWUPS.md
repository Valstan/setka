...
`⏱ 2026-08-19 · snooze 0 · fresh · код готов и задеплоен, гейт выключен — ждёт включения владельцем`
...
**Что нужно от владельца:** решить, включаем ли. Прикидка расхода: один прогон 200 постов стоил
221 420 токенов (~1100 на пост); волна отдаёт единицы-десятки кандидатов, темы идут 6 раз в сутки
по 29 регионам — порядок 50-150 тыс. токенов в сутки сверх нынешнего. Включение —
`CLASSIFIER_PREPUBLISH_ENABLED=1` в комнате КАРМАНа (имя уже в allowlist'е vault-клиента)
плюс рестарт worker.
