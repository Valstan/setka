...
- ⏳ **kirov_obl переведён с каскада на собственный пул communities (community-mode).** 2026-05-31: область теперь собирает тематические дайджесты из своих 53 областных источников (12 тем), как район, — не варясь в новостях своих же районов. Введён `region.config['digest_mode']='communities'` + чистая `_use_cascade_digest`; снят хардкод-исключение `kirov_obl` в `run_all_regions_theme`; `strict`-волны под 8 новых тем; таксономия расширена до 12 тем. Пул засеян через `/discover_communities` (нейро-классификация в чате). **Осталось после деплоя:** (1) проверить первые публикации по слотам через `/celery`; (2) `selhoz`(2)/`zdorovie`(2)/`sport`(1) — тонкие пулы, добрать точечно; (3) `tatarstan_obl` при желании тоже перевести на communities (сейчас на каскаде — backward-compat).

- 🟢 **UI: дропдаун категорий `Community.category` не содержит новых тем** (proisshestviya/molodezh/nauka/promyshlennost/selhoz/zdorovie/zhkh/priroda). Модератор пока выбирает их вручную/через discovery. Добавить в `web/templates` список из `POSTOPUS_DIGEST_THEMES`.

- 🟡 **🐞 Баг Тужи: `tuzha.vk_group_id=239050321` положительный** (у остальных регионов отрицательный) → публикация района может уходить не в ту группу. Проверить ожидаемый знак в `VKPublisher.publish_digest` / `parse_and_publish_theme` и нормализовать. Перенос из SESSION_HANDOFF (нитка «соседи/расписание»), пока не трогали.
