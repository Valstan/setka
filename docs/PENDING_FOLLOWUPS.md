...
`⏱ 2026-08-03 · snooze 0 · fresh` — код готов и смержен, **на проде применены не все шаги**.
...
1. ⏳ **`074_gateway_keys_hash.sql`** — аддитивная, применяется **до** рестарта: добавляет
   колонки и считает хэши из имеющихся значений. Старый код колонок не видит.
2. ⏳ **рестарт с новым кодом** — проверка переходит на хэш.
3. ⏳ **`075_gateway_keys_drop_plaintext.sql`** — `UPDATE ... SET secret = NULL`.
   **DESTRUCTIVE и необратимо** → гейт #025, подтверждение владельца в том же ходе.
   Предохранитель внутри: зануляются только строки, у которых хэш уже посчитан.
