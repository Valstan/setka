...
- **SSH alias `setka-prod` vs `setka`.** В `~/.ssh/config` (Windows OpenSSH) реальный alias хоста — `setka` (`3931b3fe50ab.vps.myjino.ru:49237`). Но в `CLAUDE.md`, `.claude/commands/*.md`, `.claude/settings.json` (`Bash(ssh setka-prod:*)`), `docs/REMOTE_ACCESS.md`, slash-commands `/start`/`/check`/`/celery`/`/logs`/`/sql`/`/reliz` повсеместно используется устаревшее `setka-prod`. Сегодня при первом prod probe и при релизе #13 все попытки `ssh setka-prod` падали с `Could not resolve hostname`. Sweep по всем `setka-prod` → `setka` (либо добавить host-alias `setka-prod` в `~/.ssh/config`, но это правит пользовательскую систему — не репо). Выявлено 2026-05-23.
