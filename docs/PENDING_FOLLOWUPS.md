...
- ✅ **PR 1 — backend** ([#39](https://github.com/Valstan/setka/pull/39)): миграция 012 (`community_candidates.ai_is_relevant`), `vk_search.py` принимает `localities`/`keywords` из `region.config`, hard relevance-filter (substring по корню топонима с word-boundary слева), сортировка `(matched_localities desc, members_count desc)`. Backwards-compat: пустой config → fallback на старое поведение с `CATEGORY_KEYWORDS`. +34 теста.
- ✅ **PR 2 — UI «Подготовка района»** ([#40](https://github.com/Valstan/setka/pull/40)): новая страница `/regions/<code>/prepare` между wizard'ом и `/discovery`. Два блока: localities (OSM Overpass auto-suggest + clipboard-prompt fallback) и discovery_keywords (clipboard-only). API: `GET/PATCH /api/discovery/regions/{code}/config`, `GET /api/discovery/osm-localities`. Wizard после create редиректит на `/prepare`. На `/discovery` кнопка «Настройки discovery». +24 теста.
- ⏳ **PR 3 — AI-batch через clipboard** (этот): новая страница `/regions/<code>/discovery/ai-batch`. Чанками по 30 кандидатов: программа собирает JSON + готовый prompt, юзер копирует в ChatGPT/Claude в браузере, копирует ответ обратно. Robust JSON parser (strip markdown fences, regex-fallback на первый `[...]`). API: `GET /api/discovery/regions/{code}/ai-batch`, `POST .../ai-batch/apply`, `GET .../ai-batch/status`. На `/discovery` — кнопка «🤖 AI batch», badge «✓ районный»/«✗ не районный» в карточке кандидата, фильтр «скрыть нерелевантных» (включён по умолчанию).
