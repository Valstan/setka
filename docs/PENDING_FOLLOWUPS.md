...
- 🟢 **Браузер-верификация владельцем (реальная внешняя доставка):** вкладка «Выводы» → добавить свой
  Telegram (написать радар-боту / добавить его в канал) → «тест доставки» → пришло; то же для VK.
  Внутренние endpoint'ы и пауза уже проверены смоуком выше.
