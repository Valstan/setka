...
- 🟢 **`tatarstan_obl` → community-mode** (опц.): сейчас на каскаде (backward-compat). Перевод — выставить `regions.config->>'digest_mode'='communities'` + засеять пул через `/discover_communities`. **Блокер публикации:** нужен community-токен `COMM_239149826` (vk.com/tatar_stan_info) через `/tokens` (см. token routing ниже).
