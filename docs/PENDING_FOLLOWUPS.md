...
**Главная находка.** `hold` — 11 462 поста, **25.9% потока** — это не сомнение сети, а неприменение
уже существующих правил. Проверено, что правила доезжают (эффективные постулаты 19,5 КБ, все 17 выученных
на месте): для 11 смысловых групп из 14 однозначное правило есть, а модель всё равно уходит в `hold`,
потому что это бесплатный выход без ограничений. Тот же механизм ловили 07-07 — но починили только
категорию «коммерция» припиской «НЕ по умолчанию hold», а дыра была общей. Вторая находка: тема
«новости» работает свалкой — настоящих новостей в пласте `hold/новости` около четверти.

**Сделано:** 5 черновиков отправлено (`recorded: 5`) — гейт на `hold`, тема по жанру, областной
центр, военные ЧП только из официального источника, разносол vs кликбейт. Плюс 2 правки базового
файла `config/classification_postulates.md` (этот PR): пропавшие люди → publish; родной язык района не признак мусора.
...
- 🟡 **После утверждения — отправить в retire выученное правило id 20** («Областной уровень vs чужой
  район»): новый черновик про областной центр его замещает. Панель не умеет редактировать —
  только добавить и отправить в retire двумя ходами. Оставить оба = два правила про одно в промпте.
