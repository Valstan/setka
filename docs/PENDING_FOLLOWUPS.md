...
- ⏳ **C — учёт оплат/публикаций (бывшая фаза 3 CRM). Бэкенд готов 2026-06-04 (PR в работе), UI — следующий PR.** `ad_clients`/`ad_payments`/`ad_publications` (ключ `author_vk_id`), связь заявка/пост→клиент, воронка detected→contacted→scheduled→published→paid. Задел в `ad_scheduled_posts.client_id`/`price` (миграция 025, nullable, без FK).
  - **Бэкенд (этот PR):** миграция 027 (`ad_clients` с уник. `author_vk_id` + `stage`; `ad_payments` FK→client CASCADE; `ad_publications` FK→client SET NULL; `ad_requests.client_id` + FK; FK на `ad_scheduled_posts.client_id`, обещанный в 025). ORM-модели `AdClient`/`AdPayment`/`AdPublication` + `AdRequest.client_id`. API `web/api/ad_crm.py` (`/api/ad-crm`): clients CRUD + агрегаты (оплачено/публикаций скаляр-подзапросами), `upsert-from-request/{id}` (свод заявок предложки+ЛС в одну карточку по `author_vk_id`), payments add/delete (продвигает клиента в `paid`), publications add/delete (продвигает в `published`, не понижает `paid`), `GET /funnel` (число клиентов по стадиям + деньги/публикации итого). +21 тест. **Деплой:** миграция 027 + restart `setka` (web). Воронка-стадии меняет оператор/события; авто-продвижение мягкое (не понижает уже оплаченного).
  - 🟢 _Остаток C:_ UI-страница (карточки клиентов / воронка / ввод оплат и публикаций), кнопка «Завести клиента» на карточке заявки `/ad-cabinet` (дёргает `upsert-from-request`), привязка `ad_scheduled_posts.client_id`/`price` из composer'а планировщика.
