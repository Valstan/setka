...
### 🔑 Конвейер ВК → вмалмыже.рф — упёрся в отсутствие токена КАРМАНа (MANDATE brain 2026-07-26/28)

`⏱ 2026-07-28 · snooze 0 · parked` — **условие расконсервации проверяется одной командой:**
`GET /api/secrets` с токеном комнаты отдаёт `200`. Сейчас команду нечем выполнить.
...
выдать read-write токен → положить в `/etc/setka/setka.env` как `SECRETS_TOKEN`. После этого
`GET /api/secrets?key=VMALMYZHE_INGEST_KEY` заработает, а `404` начнёт значить «портал ещё не
зеркалировал», как и задумано в grant-контракте.

Отчёт отправлен: `mailbox/to-brain/2026-07-28-mandates-report-key-blocked-smoke-path-exists.md`.
