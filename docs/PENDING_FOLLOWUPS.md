...
- ⏳ **Раунд 1 (2026-06-13) — авто-приветствие рекламодателю** (ускорить первый отклик). **Построено**
  ...
  ⏳ _Остаток:_ перенести текст в шаблон БД `ad_greeting` (правится через `/templates` в браузере,
  UTF-8 по HTTP) и **снять `AD_AUTO_GREETING_TEXT` из env** — env имеет приоритет над шаблоном, пока
  ключ на месте, шаблон не заработает. Решение владельца 2026-08-08.
