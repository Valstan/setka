...
- 🟡 `⏱ 2026-07-29 · snooze 0 · fresh` **Гейта на утечку секретов не существует вовсе** (gitleaks /
  detect-secrets / `detect-private-key`). Это **видимое** отсутствие, а не дыра класса #104, но
  дешёвая страховка: `register_oidc_client.py` и `issue_gateway_key.py` печатают секреты в stdout,
  правило проекта — «секреты только в `/etc/setka/setka.env`», а история переписки с соседями идёт
  через репозиторий. Включать **отдельной веткой** (п.5 мандата: новые правила на живом коде почти
  наверняка вскроют ворох существующих совпадений — тестовые ключи в фикстурах, примеры в доках),
  критичное правило сразу `error`, шумное — под разбор. Приёмочный критерий тот же: **показать
  красный прогон**, иначе гейт не считается заведённым.
