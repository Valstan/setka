...
- ⏸ **Следующие батчи:** создать VK-группы «- ИНФО» для оставшихся 24 районов (Суна,
  ...
  **Почва под следующую порцию готова (2026-07-26):** проверено через VK API — новых «- ИНФО»
  групп нет ни у VALSTAN (38 администрируемых), ни у МАМЫ (21), ни в подписках, то есть
  активировать пока нечего. Подготовлены: чек-лист [`docs/ops/region-activation-checklist.md`](ops/region-activation-checklist.md)
  (параметры групп сняты с живых групп батча №1 через `groups.getById`, не по памяти) и
  идемпотентный [`scripts/activate_regions.py`](../scripts/activate_regions.py) — отчёт готовности
  по каждому району (группа в VK / пул / community-токен / neighbors / права токена), запись
  только с `--apply`. Следующая порция выбрана **связным кустом**, а не по размеру пула:
  `omutninsk` (43) + `belholunitsa` (40) + `verhnekame`→Кирс (33) + `afanasyevo` (30) — все
  граничат друг с другом, поэтому соседский обмен заработает сразу внутри группы.
  ⏸ Ждём: владелец создаёт 4 группы в VK → выпускает `COMM_<id>` → миграция 073 (neighbors,
  требует его прочтения глазами) → `activate_regions.py --apply` под гейтом #025 → smoke.
