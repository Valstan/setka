...
### Discovery trigger длится >180s — nginx обрывает клиента (полу-фикс)

Обнаружено 2026-05-25 evening при smoke на tuzha (3-й инцидент за день: 09:38/17:33/23:09). Полный pipeline на 54 localities × 50 keywords = 3784 групп через VKClient.GLOBAL_PARSE_INTERVAL_SECONDS=0.4 + 150 wall.get + Groq → **268с**. Nginx обрывал клиента на 180s (HTML 504), хотя FastAPI доводит работу до конца и пишет кандидатов в БД.

**Применено как полу-фикс (2026-05-25 evening):** `/etc/nginx/conf.d/setka.conf` для location `/api/discovery/trigger`:
- `proxy_read_timeout 180s → 600s`
- `proxy_send_timeout 180s → 600s`
- Backup в `setka.conf.bak.20260525-…`, `nginx -t` + reload — OK.

Это «костыль» — UI всё равно блокируется на минуты. **Правильное решение:** перевести trigger на Celery:
- Endpoint возвращает `task_id` сразу (`202 Accepted`).
- UI polls `/api/discovery/task/{id}/status`.
- Worker делает ту же работу через `tasks/discovery_tasks.run_discovery_for_region` (он уже Celery-совместим).

В репо изменение nginx **не записано** (config на VPS, не в setka-репо). История правки в `/etc/nginx/conf.d/setka.conf.bak.20260525-…`.
...
### Discovery: omonym-prompt safety + Celery rewrite

- ~~**Relevance-фильтр пропускает омонимные стемы**~~ Закрыто 2026-05-25 (PR #44, `a7bec89`): `_passes_relevance` с center-stem requirement + ≥2 distinct stems fallback + `_LARGE_GROUP_MEMBERS_THRESHOLD=50000` для крупных пабликов. 278 ложно-релевантных групп в БД для tuzha удалены SQL'ом.
- **🟢 ChatGPT-prompt для localities — попросить помечать омонимные нп.** Текущий prompt просит просто список нп Тужинского района — ChatGPT не знает что «Коробки», «Лоскуты», «Соболи», «Чугуны», «Фомино», «Самсоны» имеют конфликтующие омонимы. Идея: расширить prompt в `web/templates/region_prepare.html` («помечай нп, чьи названия могут конфликтовать с обычными русскими словами — мы используем их в substring-фильтре»). UI может подсветить их модератору.
- **🟡 Перевести `/api/discovery/trigger` на Celery + UI polls.** См. 🔴 блокер выше. После полу-фикса nginx 600s — это уже не аварийно, но UI всё равно блокируется на минуты. Правильный путь — async task + status polls.
...
- **⏳ Отслеживать F601-фикс в фильтре рекламы (мониторинг активен с 2026-05-24)** — PR #17 накатан на прод сессией 2026-05-24 (см. `DEV_HISTORY.md`). `utils/text_utils.py:122` `commercial_patterns` объединил два списка под ключом 2 (восстановил 12 потерянных price-patterns). Фильтр стал агрессивнее. `posts` таблица пустая (не используется как persistent storage), но `parse_and_publish_theme` Celery-task пишет stats `posts_filtered_advertisement` в каждый succeeded-лог.
  - **Замер 2026-05-24 ~10:30 MSK** (через `grep parse_and_publish_theme.*succeeded` + `sed posts_filtered_old/advertisement`):
    - Период «до» (2026-05-23 14:30 → 2026-05-24 00:57, 190 task'ов): `posts_filtered_old=54134`, `posts_filtered_advertisement=188`, ratio **0.347 %**.
    - Период «после» (2026-05-24 00:57 → 10:30, 97 task'ов): `posts_filtered_old=29504`, `posts_filtered_advertisement=177`, ratio **0.600 %**.
    - Дельта: коэффициент **+73 %** (с 0.347 % до 0.600 %), но абсолютные числа сопоставимы (188 vs 177 за 10.5 ч vs 9.5 ч). Это умеренный рост, не катастрофа. Большинство task'ов всё ещё имеют `posts_final_count=0` из-за других фильтров (dedup, regions), а не из-за рекламы.
    - **Решение:** оставить вес price-patterns = 2, продолжить мониторинг ещё 24-48 ч (через тот же подход). Если ratio превысит 1.5 % или появятся жалобы на отбраковку валидных постов — снизить вес до 1.
  - **Что пока невозможно** без правки кода: посмотреть конкретные тексты отбракованных постов — `pipeline.py:111` логирует `Post filtered by...` через `logger.debug`, не пишется в проде. Чтобы сэмплировать ложно-позитивы нужно либо: (a) временно поднять log-level до DEBUG для `modules.filters.pipeline`, (b) добавить `Post.rejection_reason` колонку и сохранять отбракованные посты, (c) сделать `web/api/parsing` dry-run endpoint, который возвращает причины.
