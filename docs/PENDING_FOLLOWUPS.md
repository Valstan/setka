...
### 🔑 VK-токены — единый источник (БД) + каскад публикации + самолечащееся чтение (решения владельца 2026-07-12)

`⏱ 2026-07-12 · snooze 0 · fresh`
...
- 🟢 **За владельцем (нужно для полной силы каскада):** (1) свежий токен МАМА
  в `/tokens` (старый отвалился); (2) подписать МАМУ на 16 пабликов и назначить
  **редактором** руками в VK (Управление → Участники → Руководители) — иначе
  VK не примет от неё wall.post при отказе community-токена и VALSTAN.
  Вите редакторство больше НЕ нужно (из публикаторов выведена).
