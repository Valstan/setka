...
**Порядок перехода (не пройден):** сверка на общих постах → выключить рутину → поднять флаг →
через неделю удалить рутину. ⏳ **Следующий шаг — прогнать сверку и посмотреть на расхождения по
`action` глазами.**
