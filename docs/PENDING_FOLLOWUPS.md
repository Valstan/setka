...
`⏱ 2026-08-18 · snooze 0 · fresh · СМЕРЖЕНО (#477), ЖДЁТ ДЕПЛОЯ — на проде пока старый код`
...
- ⏳ **Деплой и проверка на проде.** После выката проверить живьём: вход по паролю с
  `сарафан.вмалмыже.рф` держит сессию; `вход.вмалмыже.рф/tokens` уводит на сарафан;
  `вход.вмалмыже.рф/login` и `/oidc/authorize` остались на месте; `POST /api/gateway/call` с
  техдомена по-прежнему отдаёт 200 (шлюз соседей). Проверять **отдельно анонимом и отдельно
  аутентифицированным** — правило 5 чек-листа `HOSTS.md`.
