...
**Дальше:** взгляд владельца на черновики → следующий прогон покажет правила в деле → кусок C (ключ
...
**Осталось:** накатить 079 и прогнать `canonicalize` по накопленным 1992 строкам — UPDATE на проде,
гейт #025.
...
**Осталось (под гейтом #025, нужен явный «да» владельца):** дописать 30 строк в `applied_migrations`
тем же способом, каким это сделали для 078 (`INSERT ... ON CONFLICT DO UPDATE`, sha из `status`).
Операция — запись в служебную таблицу, схему и данные не трогает, но это DML на проде.
