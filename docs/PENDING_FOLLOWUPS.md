...
### 🌍 Экспансия на всю Кировскую область — батч №1 (yaransk/sanchursk/kiknur) ждёт групп VK (заказ владельца 2026-07-18)
...
Батч №1 (Яранск, Санчурск, Кикнур) **скелетно готов** (миграция 061 применена, пулы засеяны:
30/14/8 доноров, брендинг+localities в region_configs, регионы `is_active=false`, `vk_group_id NULL`).
Журнал — `docs/REGION_REFRESH_LOG.md` (запись 2026-07-18).

- ⏸ **Owner после 20.07 (разбан VALSTAN):** создать VK-группы «ЯРАНСК - ИНФО», «САНЧУРСК - ИНФО»,
  «КИКНУР - ИНФО» с основного аккаунта, **порциями 2-4/день** (антиспам). Решение НЕ создавать
  через МАМУ зафиксировано (единственный publish-токен — риск заморозки остановил бы все публикации).
...
- 🟡 **Добор пробелов пулов:** yaransk — detsad/sosed 0; sanchursk — администрация округа в VK
  не найдена, detsad/reklama 0; kiknur — администрация не найдена, пул тонкий (8). Добирать
  локальным знанием / повторным сканом после запуска.
- 🟢 **Батч №2+:** остальные ~24 района (Суна, Богородское, Кумёны, Оричи, Котельнич, Яранск-соседи
  запада закрыты…) — по канону из `REGION_REFRESH_LOG.md`, тем же конвейером.
