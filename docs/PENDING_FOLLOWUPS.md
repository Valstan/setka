...
- 🟡 **Ёмкость рутины-классификатора кончилась — покрытие сползло 96% → 87%** (замер 2026-07-22):
  за 3 дня собрано **4665** постов, размечено **4044**, бэклог **621**. Рутина берёт ровно
  `limit=200` **каждый** прогон (упирается в потолок), прогоны — раз в 2 часа, 8 раз в сутки
  (04:2x–18:2x UTC = 07:2x–21:2x MSK) ⇒ потолок ≈ **1600 вердиктов/сутки** против притока
  ≈ **1555 постов/сутки**. Запаса нет: любой всплеск сбора (новые районы!) сразу наращивает
  бэклог, а с батчем №1 район добавился и дальше их будет ещё 24.
  **Рычаг на стороне владельца, не в коде:** серверный `CLASSIFIER_PENDING_MAX=200` поднимать
  ...
  NB о времени: БД пишет **UTC**, логи приложения — **MSK**; сравнивать одно с другим — верный
  способ придумать несуществующий сбой (проверено на этом же замере).
