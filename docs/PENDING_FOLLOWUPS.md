...
- **Инкрементально ломать длинные строки, помеченные `# noqa: E501`** — PR 2 закрыл E501 через массовый noqa, но real-формат можно улучшать постепенно. Прогресс:
  ...
  - **Осталось: 71 noqa в 42 файлах.** Следующие густые: `tasks/vk_carousel_tasks.py` (4), `modules/service_activity_notifier.py` (4), `scripts/run_production_workflow.py` (4), `modules/test_info_post_collector.py` (3), `modules/vk_monitor/carousel_manager.py` (3), `scripts/test_ai_groq.py` (3), `scripts/test_parse_run.py` (3), `tasks/production_workflow_tasks.py` (3), остальные 33 файла по 1-2 noqa.
