...
- ⏳ **Облачная рутина (№2):** локальная задача `setka-hitl-classifier` создана (Claude Desktop «Scheduled»,
  cron `0 */3`); ждёт: вписать ключ в её `SKILL.md` + «Run now». Крутится, пока приложение открыто (не 24/7).
