...
- ⏳ **Прод:** нужен `ECOSYSTEM_KEY` в `/etc/setka/setka.env` + `RADAR_INVITE_CODE` (снимает
  единственное человеческое действие на входе в ЕСА) + деплой и рестарт web.
...
- 🔴 **Портал (вМалмыже) — зарегистрировать нельзя, и это не наша задержка.** В их репозитории
  **нет ни одного auth-роута** (каталог `api/auth` отсутствует целиком, проверено чтением по
  ADR-0007). Регистрация сейчас = выдуманный нами `redirect_uri` в проде, то есть фикция.
  Порядок brain'а «Портал первым» соблюсти нельзя, пока они не пришлют реальный callback.
