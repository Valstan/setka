...
- ✅ ~~**Проверить лендинг в браузере на публичном хосте**~~ — 2026-08-02 открыт браузером
  ...
  отдаёт блоки. Побочная находка того же захода: **корень публичного домена уводил
  постороннего на `вход.вмалмыже.рф`** (login экосистемы), то есть короткий адрес
  «сарафан.вмалмыже.рф» покупателю рекламы показывать было нельзя. Починено в
  `middleware/auth_gate.py` — неаутентифицированный GET `/` на этом хосте → 302 на витрину.
