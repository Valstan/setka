...
- **Инкрементально ломать длинные строки, помеченные `# noqa: E501`** — PR 2 закрыл E501 через массовый noqa, но real-формат можно улучшать постепенно. Когда правишь файл из `_fix_e501.py`-списка по другой причине — заодно сломай длинные f-strings через implicit string concat в скобках и убери `# noqa: E501`. Самые «густые» места: `modules/system_status_notifier.py` (15 noqa), `tasks/parsing_tasks.py` (10), `tasks/vk_carousel_tasks.py` (4), `modules/service_activity_notifier.py` (4).
