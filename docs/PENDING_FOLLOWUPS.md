...
### 🌍 Экспансия на всю Кировскую область — ЗАВЕРШЕНА СКЕЛЕТОМ: все 27 районов, ждут групп VK (заказ владельца 2026-07-18)

`⏱ 2026-07-18 · snooze 0 · parked (ждёт разбана VALSTAN 2026-07-20)`
...
- ⏸ **Owner после 20.07 (разбан VALSTAN):** создать VK-группы «- ИНФО» для 27 районов (Яранск,
  Санчурск, Кикнур, Суна, Богородское, Кумёны, Зуевка, Фалёнки, Уни, Оричи, Орлов, Котельнич,
  Слободской, Кирово-Чепецк, Белая Холуница, Даровской, Свеча, Шабалино, Мураши, Юрья, Нагорск,
  Омутнинск, Афанасьево, Кирс, Луза, Подосиновец, Опарино)
  с основного аккаунта,
  **порциями 2-4/день** (антиспам). Решение НЕ создавать через МАМУ зафиксировано (единственный
  publish-токен — риск заморозки остановил бы все публикации).
- ⏳ **Затем (dev-сессия): активация** — `UPDATE regions SET vk_group_id=-<id>, is_active=true`;
  прописать `neighbors` (yaransk↔tuzha/pizhanka/sanchursk/kiknur и т.д.); community-токены
  `COMM_<id>` через `/tokens`; проверить вход в волны сбора + классификатор (env-гейты пустые —
  подхватятся сами); smoke первой сводки.
- 🟡 **Добор пробелов пулов:** yaransk — detsad/sosed 0; sanchursk — администрация округа не
  найдена, detsad/reklama 0; kiknur — администрация не найдена, пул тонкий (8); bogorodskoe —
  администрация не найдена, detsad/sport 0 (9); falenki — администрация/подслушано не найдены,
  reklama/detsad 0 (8); uni — райцентр в VK не представлен (9). Добирать локальным знанием /
  повторным сканом после запуска. + доборы соседям: «Касино, Мухачи, Заложане» (club206802565) →
...
- 🟡 **Пробелы батча №9:** luza — detsad/sosed 0, sport 1; oparino — reklama 0 (только
  мульти-спам-сети); podosinovets — sosed 0. Добирать локальным знанием / повторным сканом.
- 🟡 **Пробелы батча №8:** afanasyevo — администрация округа и спорт в VK не найдены,
  admin/sport/sosed 0; verhnekame — detsad 0 (Музей игрушки на базе д/с №5 положен в kultura);
  omutninsk — sosed 0, detsad тонкий (1). Добирать локальным знанием / повторным сканом.

### 🔑 VK-токены — единый источник (БД) + каскад публикации + самолечащееся чтение (решения владельца 2026-07-12) — ЗАКРЫТО/ЗАДЕПЛОЕНО

`⏱ 2026-07-12 · snooze 0 · fresh · осн. работа ЗАДЕПЛОЕНА+проверена live (PR #336/#337, прод a255576); остаток — только 🟢 low-prio ниже`
...
`⏱ 2026-07-05 · snooze 0 · fresh · петля обучения замкнута+задеплоена 2026-07-08 (окно 3д + ретеншн + правила overlay + веб-утверждение, PR #320/#321); остаток — оператор разбирает ленту, owner создаёт рутину-дистиллятор, 3 хвоста ниже`
...
- 🟡 **МАМА — единственный publish-токен (обнаружено 2026-07-16):** ВСЕ community-токены
  ...
  user-токене. Owner: когда будет живой админ — перевыпустить community-токены из-под
  него (и/или вернуть VALSTAN после разбана). `⏱ 2026-07-16 · snooze 0 · watch (ждёт
  разбана VALSTAN / нового админа)`
- 🟡 **Watch: успевает ли рутина за мультирегиональным потоком** — поток ~1000+ постов/волна
  против ёмкости рутины ≤200×16=3200/день; смотреть `/api/classifier-review/health`
  (backlog, coverage_pct) через пару дней. Если не успевает — поднимать частоту рутины или
  ускорять Enforce v2 (Claude API). Токен-расход рутины виден только на claude.ai (owner).
  `⏱ 2026-07-16 · snooze 0 · fresh`
...
- 🟡 `⏱ 2026-07-08 · snooze 0 · fresh` **Системный риск.** Инцидент Уржум 2026-07-08: рекламная сводка
