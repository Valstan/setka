...
- **GRANT в миграциях / ALTER DEFAULT PRIVILEGES.** Миграция 008 (этап 4b) ПРИ ПЕРВОМ применении сломала прод — таблица создалась с owner=postgres, а `setka_user` не унаследовал прав. Hot-fix: добавил `GRANT ALL ... TO setka_user` в саму миграцию (см. DEV_HISTORY 2026-05-22). Правильное решение — один раз настроить `ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO setka_user; ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO setka_user;` и больше не таскать GRANT по каждой миграции.
