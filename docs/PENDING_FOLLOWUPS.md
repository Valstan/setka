...
Три блока расширения. **Блок B (планировщик отложки) — B1 реализован 2026-06-04**, ждёт деплоя.

- ⏳ **B1 — планировщик отложенных постов (готов, НЕ задеплоен).** Из кабинета формируется график постов по датам → нативная VK-отложка (`wall.post publish_date`), VK сам публикует. PR-цепочка на `main`: [#131](https://github.com/Valstan/setka/pull/131) seam (`publish_date`/`signed`/`set_post_comments`/`vk_wall_photo_upload`), [#132](https://github.com/Valstan/setka/pull/132) таблица `ad_scheduled_posts` (**миграция 025**) + API `POST/GET /scheduled` + `POST /scheduled/{id}/cancel` (`wall.delete`), [#133](https://github.com/Valstan/setka/pull/133) UI composer (мультидата + тумблеры from_group/signed/комменты + календарь). **🔴 Деплой: применить миграцию 025 + restart web/worker/beat через `/reliz`.** Браузер-верификация за владельцем. Известный нюанс: создание N постов в одну группу подряд тормозит на `POST_INTERVAL_SECONDS=5` (для 5 дат ≈20с) — приемлемо для MVP, при желании вынести в фон.
- 🟢 **B2 — из предложки в отложку (in-place).** Кнопка «Запланировать» на заявке: `wall.edit(owner_id, post_id, publish_date=…)` **редактирует предложенный пост на месте** (сохраняет «Предложил(а): автор» и подпись — owner-требование), + контролы `signed`/комментарии. ⚠️ Требует **живого VK-probe**: подтвердить, что `wall.edit`+`publish_date` на suggested-посте сохраняет атрибуцию и планирует (а не публикует сразу). Зависит от B1.
- 🟢 **A — реклама во входящих ЛС + диалог из кабинета.** beat-детект рекламы в `messages.getConversations` → заявки `ad_requests` с `origin='inbound_dm'` (новая колонка); тред-вью (`messages.getHistory` — добавить в VKClient) + ответ из кабинета (готовый `send_message` + картинки). Сейчас ловится только предложка.
- 🟢 **C — учёт оплат/публикаций (бывшая фаза 3 CRM).** `ad_clients`/`ad_payments`/`ad_publications` (ключ `author_vk_id`), связь заявка/пост→клиент, воронка detected→contacted→scheduled→published→paid. Задел в `ad_scheduled_posts.client_id`/`price` (миграция 025, nullable, без FK).
