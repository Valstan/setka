...
- ⏳ **Valstan заблокирован VK на 24ч** — после релиза PR с TokenPolicy выключить руками: либо кнопка «Выкл. на 24ч» на `/tokens`, либо `POST /api/tokens/VALSTAN/disable?hours=24`, либо `UPDATE vk_tokens SET disabled_until=now()+'24h' WHERE name='VALSTAN'`. До восстановления: парсинг через Vita автоматически; wall.post дайджеста — через community-токены конкретных групп; wall.repost (copy_setka) — НЕДОСТУПЕН (VK API не принимает community-токен).
