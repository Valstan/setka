...
- **`scripts/setup-dev.ps1` хардкодит `py -3.11`** — если 3.11 не установлен, скрипт падает с ошибкой. На локалке у @valstan стоит только 3.12 (=прод), пришлось обходить скрипт руками (2026-05-23, dev-worktree bootstrap). Стоит сделать fallback: если `py -3.11` недоступен → попробовать `py -3.12` с warning'ом. Аналогично `setup-dev.sh`.
