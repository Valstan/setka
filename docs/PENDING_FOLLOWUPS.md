...
- 🟢 `⏱ 2026-07-26 · snooze 0 · fresh` **Мёртвые строковые имена задач в UI-подписях** (найдено той же сверкой): `modules/celery_task_monitor.py::_format_task_name` и `modules/system_status_notifier.py::_format_task_name_for_user` держат словари человекочитаемых названий на **8 имён, ни одно из которых не зарегистрировано** (`tasks.notification_tasks.*`, `tasks.publishing_tasks.*`, `tasks.real_vk_workflow.*`, `tasks.analysis_tasks.*`, `tasks.production_workflow_tasks.*`, `tasks.monitoring_tasks.*` — модулей нет в репо). Функционально безвредно (`.get(name, fallback)` → показывается последний сегмент имени), поэтому в жёсткий гейт эти словари **сознательно не включены**: там имя — ключ поиска, а не вызов. Чистка — отдельным мелким PR.
