...
**Что сделать.** Нормализация темы **на запись** в `record_verdicts`: trim + casefold + словарь
синонимов → канон; неизвестное значение оставлять как есть, но логировать (молча подгонять поток
запрещено тем же доводом, что и в конвейере). Ключевой вопрос — **где живёт словарь синонимов**:
в 070 он был временной таблицей внутри миграции и умер вместе с ней. Варианты: durable-таблица
`classifier_theme_aliases` с редактором рядом с `add_theme`/`reassign_theme`, либо константа в коде.
Плюс разовый прогон `reassign_theme` по 29 накопившимся значениям — это UPDATE на проде, гейт #025.
