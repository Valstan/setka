...
- 🟡 **Офферные картинки кладёт владелец** в `web/static/ad_offers/*.jpg|png` (в git не коммитятся, см. `.gitignore` там). Без community-токена группы картинки в ЛС не уйдут (R4) — оффер уйдёт текстом. UI-управление наборами — фаза 2.
- 🟡 **Правки текста в textarea кабинета не сохраняются перед `/send`** — отправляется сохранённый `prepared_message` (из `prepare`). Если оператор правил вручную — нажать «Подготовить» снова либо «Копировать» и отправить с личного аккаунта. TODO: endpoint сохранения отредактированного `prepared_message`.
...
- 🟢 **Фаза 2** — UI-управление офферными картинками (наборы по регионам), bulk-действия в инбоксе, авто-send где `is_allowed=1` (под контролем оператора).
