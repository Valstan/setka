...
- **Инкрементально ломать длинные строки, помеченные `# noqa: E501`** — PR 2 закрыл E501 через массовый noqa, но real-формат можно улучшать постепенно. Прогресс:
  - ✅ **2026-05-24 PR #1**: `modules/system_status_notifier.py` — 15 noqa → 0 (см. `DEV_HISTORY.md` «Break long lines PR #1»).
  - **Осталось: 81 noqa в 43 файлах.** Следующие густые: `tasks/parsing_tasks.py` (10), `tasks/vk_carousel_tasks.py` (4), `modules/service_activity_notifier.py` (4), `modules/test_info_post_collector.py` (3), `modules/vk_monitor/carousel_manager.py` (3), `scripts/test_ai_groq.py` (3), `scripts/run_production_workflow.py` (4), `scripts/test_parse_run.py` (3), `tasks/production_workflow_tasks.py` (3), остальные 33 файла по 1-2 noqa.
  - **Стратегия**: ломать f-string'ы через implicit string concat в `(...)` или выносить повторяющиеся подвыражения в локалки.
- **Рефакторинг `scripts/*` через `pyproject.toml` + `pip install -e .`** — PR 3 закрыл E402 через массовый noqa, но первопричина (`sys.path.insert(0, ...)` перед импортами) осталась. Правильный fix: завести `pyproject.toml` с `[project] name = "setka"`, перейти на editable install в venv (`pip install -e .`), убрать `sys.path.insert` из ~33 скриптов и снять `~115 # noqa: E402` в `scripts/*`. Затрагивает CI (`.github/workflows/ci.yml`), `scripts/setup-dev.{sh,ps1}`, прод-деплой (если `python3 scripts/X.py` где-то используется в systemd/cron). Большой рефакторинг — отдельная сессия. Полезный side-effect: можно будет `from setka.X import …` вместо относительных импортов.
