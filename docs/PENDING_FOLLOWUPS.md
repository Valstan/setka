...
`⏱ 2026-08-09 · snooze 0 · fresh · план отправлен, ждём слова brain по развилке`
...
Пилот — вмалмыже.рф ← «Малмыж-Инфо». План с оценкой: `mailbox/to-brain/2026-08-09-multi-site-conveyor-plan.md`.

**Переиспользуется (≈половина конвейера уже стоит):** сбор/дедуп/фильтры `vk_monitor`;
`collected_post_audit` как вход (ключ `lip` = `<owner_abs>_<post_id>` — **группа-источник уже в
ключе**, отбор «группы → сайт» без правки сбора); вся HITL-петля (вердикты, коррекции, overlay-правила
с aging, `agree_rate_stats`, `enforce.py`, лента, `/distill`); Groq-клиент; vault-клиент; beat.

**Строится — 4 куска:** A конфиг сайтов + отбор (0.5 сессии) · B медиа-URL в аудите (0.5) ·
C ключ «сайт» через петлю (1) · D доставка с ретраями и инвариантом (1). **Пилот — 2 сессии**
(A+B+D), полный конвейер — 4.
