...
- ⏳ **PR 2 — UI «Подготовка района»** (этот): новая страница `/regions/<code>/prepare` между wizard'ом и `/discovery`. Два блока: localities (OSM Overpass auto-suggest + clipboard-prompt fallback) и discovery_keywords (clipboard-only). API: `GET /api/discovery/regions/{code}/config`, `PATCH .../config/{field}`, `GET /api/discovery/osm-localities?district=`. После создания региона wizard редиректит на `/prepare` (а не сразу на `/discovery`) — юзер заполняет config и САМ запускает discovery с фильтром. На `/discovery` добавлена кнопка «Настройки discovery» для возврата к prepare.
