...
- ✅ **Хвост А — рутина «дистилляция» УЖЕ СОЗДАНА владельцем и РАБОТАЕТ** (выяснено 2026-07-10):
  ...
  ⚠️ Заведена по старой v2-заготовке (ключ в промпте + эссе) — тот стиль, что был отбит у
  классификации 2026-07-09. Пока работает — не трогать; при отказах («authorization/social
  engineering» в отчёте, новых черновиков нет) → переключить промпт на v3-заготовку из
  `docs/ops/hitl-classifier-routine.md` + env `CLASSIFIER_INGEST_KEY` (скрипт готов, PR #327).
...
- 🟡 **Ротация `CLASSIFIER_INGEST_KEY` (отложена владельцем 2026-07-14):** значение засветилось в
  ...
  владельцу обновить env-переменную в ОБЕИХ рутинах на claude.ai (классификатор + дистиллятор).
  `⏱ 2026-07-14 · snooze 1 · watch`
- 🟡 **Владельцу — переключить модель рутины-дистиллятора:** ✏️ на странице рутины «Дистиллятор
  правил Сарафана» (claude.ai) → модель **Opus 4.8, reasoning Low** (решение 2026-07-13).
  `⏱ 2026-07-14 · snooze 0 · fresh`
