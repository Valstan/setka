...
**Строится — 3 куска (не 4):** ~~A конфиг сайтов + отбор~~ **сделано и проверено на проде (PR #454)**
· ~~B медиа-URL в аудите~~ **отпал** · C ключ «сайт» через петлю (1 сессия) · D доставка с ретраями
и инвариантом (1). **Пилот — 1 сессия** (остался D), полный конвейер — 2.
