...
- ✅ **Построено 2026-06-14** (ветка `feat/krugozor-broadcast`): `modules/krugozor_broadcast.py`
  — round-robin по krugozor-источникам → копи-веер на стены всех целевых регионов с
  native-атрибуцией VK (`copyright`=ссылка на источник) + футер с именем; переиспользует
  кубики copy_setka (VKPublisher/роутинг токенов, копирование вложений, паттерн добора,
  WorkTable-состояние `copy/krugozor`), но **отдельный модуль** — copy_setka не трогаем.
  Гейт `KRUGOZOR_BROADCAST_DISABLED` (OFF по умолчанию, #008), env-конфиг (категория,
  цели, max-age, интервал, exclude-ids). Beat `krugozor-broadcast-evening` 20:00 MSK.
  Диспатч `region=copy/theme=krugozor`. +21 тест (1312 зелёных). Видео → деградация в
  ссылку (как copy_setka), фото перезаливаются.
- ⏳ **Деплой (без миграции):** `git pull` + restart worker/beat (загрузить beat-entry +
  диспатч). Гейт OFF — рассылка молчит до включения env. Через `/reliz`.
- ⏳ **Smoke на 1 районе** перед всей областью (#020): env `KRUGOZOR_BROADCAST_DISABLED=0`
  + `KRUGOZOR_TARGET_REGION_CODES=<1 район>` → ручной триггер задачи → проверить пост на
  стене (текст+фото+«Источник»-ссылка) → снять фильтр на все 16.
- 🟢 **Стат-замер охвата → решение 1× vs 2×** (эмпирическая модель): после ~недели
  посчитать просмотры/лайки krugozor-постов (по футер-сигнатуре на стенах или мини-стат-слой,
  переиспользуя `wall.getById`-стек ad-CRM С3) → зайдёт → добавить обеденный слот 13:00.
...
рекламодателем), реализую в тот же день. Следующий раунд — **~2026-06-20**: посмотреть статистику
(`/funnel`, время отклика, сколько авто-приветствий ушло) и предложить следующее.
...
- ⏳ **Раунд 1 (2026-06-13) — авто-приветствие рекламодателю** (ускорить первый отклик). **Построено**
  ...
  🟢 _Остаток:_ деплой + владелец задаёт env (сообщества + текст) + браузер/Telegram-проверка.
...
`⏱ 2026-06-07 · snooze 0 · watch (цифры сняты 2026-06-14, ack brain отправлен — ждёт решения Фазы 2 владельцем/brain; gate перед enforcing — спот-чек владельца на 19 флагах)`
...
- ✅ **Деплой** — состоялся (миграция 035 применена, `DIGEST_CURATION_SHADOW_ENABLED=1` + регион Малмыж в env, worker/beat перезапущены): подтверждено наличием накопленных прогонов в `digest_curation_runs`.
...
- ⏳ **Решение Фазы 2 (за владельцем/brain):** enforcing с fail-open vs перенос фильтра в код на Haiku-API (токены не блокируют — ~530/прогон; решает желаемая автономность от desktop'а). **Gate перед enforcing — спот-чек владельца на 19 флагах** (precision его глазами). Граница автономии: LLM только вердикт, публикация/удаление — за кодом (#025/#027).
