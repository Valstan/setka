...
- 🟢 **Хвост Б — beat-джоба снапшота overlay → git-файл** (аудит/откат выученных правил): периодически
  сериализовать `approved` из `classification_rules` в `config/classification_postulates.md` (или companion),
  чтобы выученный слой был в git-истории (ADR-0005 обещает снапшот; сейчас ручной). Не блокер.
- 🟢 **Хвост В — подсветка «протухших» правил** по `last_effective_at` (aging #033): правило без срабатываний
  N месяцев → кандидат на `retire`; поле пишется, UI-подсветки/авто-предложения пока нет.
- 🟢 **Старый хвост облачной рутины:** (1) отключить локальную задачу `setka-hitl-classifier` (`enabled:false`),
  чтобы не дублировала облачную; (2) снять коннектор Gamma с рутины (лишний контекст).
