...
- 🟢 **v2-бэклог (осталось):** запись в VK (guarded, per-key scope); async-джоба для тяжёлого «прочесать
  весь паблик» (как `/api/parsing`); MCP-обёртка для проектов-потребителей.
...
`⏱ 2026-06-07 · snooze 0 · watch (спот-чек владельца пройден 2026-06-29: precision 3/19 ≈ 16% → enforcing текущей рубрики НЕ оправдан; развилка переоснастить/свернуть — за владельцем/brain)`
...
- ⏳ **Решение Фазы 2 (за владельцем/brain) — развилка после спот-чека:** **enforcing текущей рубрики
  НЕ включаем** (при 16% срезал бы 16 нужных ради 3 — measure-before-enforce сказала «стоп»).
  **(A)** переоснастить рубрику на интент владельца (drop только мульти-регион вербовка/спам +
  распущенно-военное + отдельная реклама; ось «не-район» убрать) → перемерить на свежем окне shadow;
  **(B)** свернуть LLM-фильтр релевантности (стенс «разносол» ближе к разрешающей логике алгоритма).
  Shadow-таблица `bulletin_curation_runs` остаётся аудитом в обоих случаях. Граница автономии (если
  всё же enforcing): LLM только вердикт, публикация/удаление — за кодом (#025/#027).
