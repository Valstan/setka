...
Саморег был выключен ровно одной переменной: `web/api/auth.py` требует
`RADAR_INVITE_CODE`, а её не было в `/etc/setka/setka.env` → любой `register`
отдавал 403. То есть потребитель **не мог завести себе смоук-аккаунт сам** —
и именно это, а не «нужен браузер с живым VK», делало первую проверку дорогой.

## Снято 2026-07-29 — узкого места больше нет

`RADAR_INVITE_CODE` задан на проде (ADR-0010, мандат brain 2026-07-28). Теперь
шаг «получить аккаунт» — такой же curl, как остальные четыре:

```bash
curl -s -X POST "$ESA/api/auth/register" -H 'Content-Type: application/json' \
  -d '{"login":"<ваш-смоук-логин>","password":"<пароль>","invite_code":"<код>"}'
# → {"ok":true,"role":"radar","login":"…"}
```

Роль такого аккаунта — `radar` (лента событий), без операторских прав:
эскалации через саморег нет. Инвайт-код лежит в вашей vault-комнате рядом с
экосистемным ключом.

Регистрация самого клиента (шаг 1 выше) тоже стала self-serve —
[`ecosystem-self-serve.md`](ecosystem-self-serve.md), один curl.

**Круг проверен целиком 2026-07-29** на клиенте `portal`: саморег → логин →
`/oidc/authorize` → код → `/oidc/token` (access + id + refresh, scope
`openid profile`) → `/oidc/userinfo` (`sub`, `name`). Ни браузера, ни VK.

> Нюанс проверки: сессионная кука ставится на домен `.вмалмыже.рф`
> (`SESSION_COOKIE_DOMAIN`), поэтому прогонять круг с `127.0.0.1` бесполезно —
> кука не вернётся, и `authorize` уведёт на `/login`. Гоняйте по публичному
> issuer'у, как в примерах выше.
