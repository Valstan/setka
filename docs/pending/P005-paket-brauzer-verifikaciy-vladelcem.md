...
- **Ротация `COMM_137760500` (из P084, утечка в лог 19.08):** «Управление → Работа с API» группы 137760500 → новый
  ключ → заменить на `сарафан.вмалмыже.рф/tokens` → проверить `poll_radar_vk_intake` (`ok: True`). Логи уже чисты.
