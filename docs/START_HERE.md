...
**Основной способ — SSH** к хосту, где развёрнут SETKA (`/home/valstan/SETKA`). Он гибче shell, git, systemd, логи, отладка.

**MCP в Cursor** — **вторичный** (запасной) канал; использовать только если SSH недоступен или для узкого сценария. **Не подключаться к MCP других проектов/VPS** и не считать их сервером SETKA.

Полное правило, проверка хоста и примеры: **[`REMOTE_ACCESS.md`](REMOTE_ACCESS.md)**.
