...
**Date:** 2026-06-30
**Status:** Proposed (design-first; постройка после ратификации контракта + ответов владельца)
**Drives:**
...
**Публичные эндпоинты** (issuer, напр. `https://id.малмыже.рф` — A-запись на хост Радара):
...
## Открытые вопросы владельца (НЕ блокеры, нужны до постройки)

1. **Нейминг:** подтвердить зонтик «Радар = платформа Сарафан», модули Радар-ID (SSO) / Радар-лента (контент).
2. **Публичная поверхность:** переиспользовать `id.малмыже.рф` (A-запись → хост Радар-SSO, не бокс GONBA) + TLS на хосте setka.
3. **Пилотный клиент Ф1:** GONBA или trener (оба готовы).
