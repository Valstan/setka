...
Поля: `Region.kind` (`raion`/`oblast`/`strana`), `Region.parent_region_id` (FK self-ref). Каскадный дайджест для `oblast`/`strana` — `modules/cascaded_digest.py`. Полный список регионов — в таблице `regions` в БД.
