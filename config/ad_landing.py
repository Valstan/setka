...
PIN_PRICE_RUB = 200
# Безлимит на 30 дней (Этап 2): продукт вне лесенки PACKAGES — туда не добавлять,
# quote_price берёт первый covers=None как потолок «вся сеть».
UNLIMITED_PRICE_RUB = 5000
...
EXTRAS: List[str] = [
    ...
    f"Безлимит на 30 дней — {UNLIMITED_PRICE_RUB} ₽: любые сообщества сети, до одного поста "
    "в сутки в каждом.",
    "Поможем бесплатно: подскажем, в каких районах живёт ваша аудитория.",
    "После оплаты можем выписать чек как самозанятый — по запросу.",
]
...
# Чек самозанятого — одна строка, которую видят все каналы продажи
# (лендинг, кабинет, ВК-бот, подтверждение оплаты). Заказ владельца 2026-09-12.
RECEIPT_NOTE = "После оплаты можем выписать чек как самозанятый — просто скажите, что нужен."
