...
def get_bulletin_fresh_hours() -> float:
    """Граница «свежего» поста в часах для порядка публикации (дефолт 24).
    ...
    Это НЕ окно сбора: посты старше ``max_post_age_hours`` (дефолт 72 ч) в волну
    не попадают вовсе, а здесь делится то, что уже прошло. 0 отключает деление.
    """
    try:
        v = float(_getenv("BULLETIN_FRESH_HOURS", "24") or "24")
    except ValueError:
        return 24.0
