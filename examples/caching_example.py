...
from utils.cache import cache, get_cache, invalidate_cache
...
async def get_all_regions(db: AsyncSession = Depends(get_db_session)):
    ...
    return [{"id": r.id, "code": r.code, "name": r.name, "is_active": r.is_active} for r in regions]
...
async def get_posts_by_region(
    region_code: str, limit: int = 50, db: AsyncSession = Depends(get_db_session)
):
    """
...
async def get_communities_by_region(
    region_code: str, category: str = None, db: AsyncSession = Depends(get_db_session)
):
    """
...
def _get_cache_recommendations(stats: dict) -> List[str]:
    ...
    if hit_rate < 50:
        recommendations.append(
            "⚠️ Low hit rate (<50%). Consider increasing TTL or caching more endpoints."
        )
    elif hit_rate > 90:
    ...
    if keys_count > 10000:
        recommendations.append(
            "⚠️ Many keys in cache. Consider shorter TTL or pattern-based cleanup."
        )
...
async def clear_cache(pattern: str = "*"):
    ...
    return {"success": True, "pattern": pattern, "keys_deleted": deleted}
...
async def search_posts(q: str, limit: int = 50, db: AsyncSession = Depends(get_db_session)):
    """
...
async def warmup_cache(db: AsyncSession = Depends(get_db_session)):
    ...
    return {"success": True, "keys_warmed": warmed}
...
async def demo_performance_comparison(db: AsyncSession = Depends(get_db_session)):
    ...
    # Test without cache
    start = time.time()
    result = await db.execute(select(Post).order_by(Post.date_published.desc()).limit(100))
    posts = result.scalars().all()
