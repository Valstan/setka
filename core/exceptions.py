...
class VKAPIException(SetkaException):
    ...
    def __init__(
        self, message: str, error_code: int = None, retry_after: int = None, method: str = None
    ):
        self.error_code = error_code
        self.retry_after = retry_after
        self.method = method

        details = {"error_code": error_code, "retry_after": retry_after, "method": method}
        super().__init__(message, details)
...
class VKRateLimitException(VKAPIException):
    ...
    def __init__(self, retry_after: int = 1):
        super().__init__(
            message="VK API rate limit exceeded", error_code=6, retry_after=retry_after
        )
...
class VKTokenInvalidException(VKAPIException):
    ...
    def __init__(self):
        super().__init__(message="VK token is invalid or expired", error_code=5)
