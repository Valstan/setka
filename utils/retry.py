...
def retry_vk_api(max_attempts: int = 3):
    ...
    return retry(
        ...
        retry=retry_if_exception_type(
            (VKRateLimitException, asyncio.TimeoutError, ConnectionError)
        ),
...
async def retry_with_fallback(
    primary_func: Callable, fallback_func: Callable, max_attempts: int = 3, *args, **kwargs
) -> Any:
    """
...
async def retry_with_circuit_breaker(func: Callable, circuit_breaker, *args, **kwargs) -> Any:
    """
    ...
    if not circuit_breaker.is_closed():
        raise SetkaException(
            f"Circuit breaker is OPEN for {func.__name__}", details={"state": circuit_breaker.state}
        )
...
async def log_error_to_db(
    component: str, error_type: str, error_message: str, details: dict = None
):
    """
    ...
    try:
        ...
        logger.error(f"[{component}] {error_type}: {error_message}", extra={"details": details})
