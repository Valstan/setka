...
6. **Закрытие сессии** — `/finish`. Проверяет открытые/новые задачи в `PENDING_FOLLOWUPS.md`, активную нитку в `SESSION_HANDOFF.md`. Опционально — commit + PR без deploy.
...
| [`/finish`](.claude/commands/finish.md) | Закрыть сессию: проверка PENDING/SESSION_HANDOFF, uncommitted, предложить /reliz. |
| [`/close_session`](.claude/commands/close_session.md) | Финализировать сессию: обновить SESSION_HANDOFF.md (sticky-note куда мы шли), синхронизировать PENDING, отдельный handoff-коммит через PR. |
