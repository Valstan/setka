...
из DigestBuilder.build_digest при превышении max_text_length.
...
def test_truncate_text_used_by_digest_builder_bezfoto_branch():
    ...
    `TextOnlyDigestBuilder.build_bezfoto_digest`, активный метод publisher'а
    (используется для рекламных дайджестов, migrated from old_postopus
    ...
    from modules.publisher.digest_builder import TextOnlyDigestBuilder

    items = ["Длинная новость с множеством слов и подробностей. " * 5] * 10
    builder = TextOnlyDigestBuilder(
        header="📰 Тест",
        hashtags=["тест"],
        local_hashtag="#тест",
        max_text_length=200,
    )
    result = builder.build_bezfoto_digest(text_items=items, header="📰 Test header", hashtag="test")
