...
ветку хелпера загрузки фото `_upload_request_photos`: без user-токена / без URL /
с мусором → пустой список (пост уйдёт текстом, без сети). Грузим user-токеном:
VK запрещает заливку на стену под community-auth (error 27).
"""
...
def test_no_user_token_returns_empty():
    # есть URL, но нет user-токена → загружать нечем
...
def test_no_urls_returns_empty():
    assert _upload_request_photos(-123, "user-tok", []) == []
    assert _upload_request_photos(-123, "user-tok", None) == []


def test_non_http_urls_filtered():
    # нет валидных http(s)-URL → пусто, до сети не доходим
    assert _upload_request_photos(-123, "user-tok", ["not-a-url", "", None]) == []
