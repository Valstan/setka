...
from modules.notifications.vk_actions import like_comment, reply_to_comment, send_message
...
def test_like_comment_uses_only_user_token_even_with_community_configured():
    """likes.add is user-token-only — community token must be ignored to avoid
    VK error 3 ``Unknown method passed`` (regression fix 2026-05-22)."""
    with patch("modules.notifications.vk_actions.vk_api.VkApi") as m:
        user_api = MagicMock()
        user_api.likes.add.return_value = {"likes": 42}
        ...
        result = like_comment(
            ...
            comment_id=CID,
            user_token="user",
            community_tokens={abs(OWNER): "community"},
        )

    assert result["success"] is True
    assert result["likes_count"] == 42
    assert result["via"] == "user-token"
    # The community token must never reach VkApi() — only "user" should.
    init_tokens = [call.kwargs.get("token") for call in m.call_args_list]
    assert init_tokens == ["user"]
    user_api.likes.add.assert_called_once_with(
        type="comment",
        owner_id=OWNER,
        item_id=CID,
    )
...
def test_like_comment_no_community_token():
    """No community-token configured → straight to user-token (unchanged)."""
    with patch("modules.notifications.vk_actions.vk_api.VkApi") as m:
        user_api = MagicMock()
...
def test_like_comment_api_error_returns_failure():
    """ApiError from VK is captured and returned as a failure payload."""
    with patch("modules.notifications.vk_actions.vk_api.VkApi") as m:
        user_api = MagicMock()
        user_api.likes.add.side_effect = _api_error(100)
        m.return_value.get_api.return_value = user_api
...
def test_reply_to_comment_via_community_token():
    ...
    community_api.wall.createComment.assert_called_once_with(
        ...
        post_id=POST,
        message="hi",
        reply_to_comment=CID,
        from_group=abs(OWNER),
    )
...
def test_send_message_via_community_token():
    ...
    with patch("modules.notifications.vk_actions.vk_api.VkApi") as m:
        ...
        result = send_message(
            ...
            message="привет",
            user_token="user",
            community_tokens={GROUP: "community"},
            random_id=42,
        )
...
def test_send_message_handles_negative_group_id():
    ...
    with patch("modules.notifications.vk_actions.vk_api.VkApi") as m:
        ...
        send_message(
            ...
            message="x",
            user_token="user",
            community_tokens={158787639: "community"},
            random_id=7,
        )
