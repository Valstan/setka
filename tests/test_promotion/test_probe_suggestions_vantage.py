...
по первому району списка, и дальше считала его посторонним везде. На живом
прогоне 18.09 это развалилось: первым по `region_id` идёт `mi`, где все три
доступных аккаунта — свои (двое админов, третий подписчик), и скрипт отказался
мерить **все 53** сообщества вместо одного.
...
class TestMeasurePicksPerCommunity:
    ...
    def test_subscriber_is_used_when_no_outsider(self, mod, monkeypatch):
        """Случай `mi`: посторонних нет — мерим подписчиком и помечаем это."""
        answers = {
    ...
    def test_all_admins_give_nothing_not_a_false_no(self, mod, monkeypatch):
