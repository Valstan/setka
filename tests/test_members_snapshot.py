...
def test_screen_name_map_basic():
    vk_info = [
        {"id": 100, "screen_name": "malmyzh_info", "members_count": 1},
        {"id": -200, "screen_name": "kirovskaya_info"},  # знак нормализуется
    ]
    assert ms.build_screen_name_map(vk_info) == {
        100: "malmyzh_info",
        200: "kirovskaya_info",
    }


def test_screen_name_map_skips_autogenerated_and_banned():
    """club<id>/public<id> — не задан владельцем; deactivated — мимо."""
    vk_info = [
        {"id": 100, "screen_name": "club100"},
        {"id": 200, "screen_name": "public200"},
        {"id": 300, "screen_name": "banned_grp", "deactivated": "banned"},
        {"id": 400, "screen_name": ""},
        {"id": 500},
    ]
    assert ms.build_screen_name_map(vk_info) == {}
...
def test_collect_caches_screen_names_into_region_config():
    """Ночная таска кладёт красивый адрес в config['screen_name'] (для /regions/links)."""

    class _Region:
        def __init__(self, rid, gid, config=None):
            self.id = rid
            self.vk_group_id = gid
            self.config = config

    r1 = _Region(1, -100)  # адреса не было
    r2 = _Region(2, -200, {"screen_name": "old_name"})  # переименован
    orm_regions = [r1, r2]
    ...
    class _Session:
        ...
        async def execute(self, stmt):
            self.calls += 1
            r = MagicMock()
            r.all.return_value = [(1, -100), (2, -200)]  # read-фаза (кортежи)
            r.scalars.return_value.all.return_value = orm_regions  # write-фаза
            return r

    session = _Session()
    vk_info = [
        {"id": 100, "members_count": 10, "screen_name": "malmyzh_info"},
        {"id": 200, "members_count": 20, "screen_name": "new_name"},
    ]
    ...
    assert r1.config == {"screen_name": "malmyzh_info"}
    assert r2.config == {"screen_name": "new_name"}
    session.commit.assert_awaited_once()


def test_collect_screen_name_unchanged_is_not_touched():
    """Совпадающий адрес не переприсваивается — без пустых UPDATE каждую ночь."""

    class _Region:
        def __init__(self):
            self.id = 1
            self.vk_group_id = -100
            self.config = {"screen_name": "same_name", "other": "kept"}

    region = _Region()
    original_config = region.config
    ...
    class _Session:
        ...
        async def execute(self, stmt):
            r = MagicMock()
            r.all.return_value = [(1, -100)]
            r.scalars.return_value.all.return_value = [region]
            return r

    session = _Session()
    vk_info = [{"id": 100, "members_count": 10, "screen_name": "same_name"}]
    ...
    assert region.config is original_config  # тот же dict — ORM не увидит изменение
