    ...
    try:
        ...
        async def run_validation():
            ...
            for name, token in VK_TOKENS.items():
                if not token:
                    results[name] = {"is_valid": False, "error": "Token is empty"}
                    continue
                ...
                try:
                    ...
                    if user_info:
                        ...
                        results[name] = {"is_valid": False, "error": "Failed to get user info"}

                except Exception as e:
                    results[name] = {"is_valid": False, "error": str(e)}

            logger.info(
                f"Token validation completed: {len(valid_tokens)}/{len(VK_TOKENS)} tokens are valid"
            )
