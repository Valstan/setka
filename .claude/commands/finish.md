...
> Прод обновлений не делал — для деплоя в следующей сессии запусти `/reliz`. Открытые задачи и контекст подгрузятся через `/start`. Если хочешь зафиксировать «куда мы шли» (для другого компа или после длительной паузы) — запусти `/close_session`, он обновит `docs/SESSION_HANDOFF.md` отдельным коммитом.
