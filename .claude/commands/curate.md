...
SSH к проду). Данные — в **прод-БД**, поэтому скрипт гоняем через `ssh sarafan`.

## Шаг 0. Проверить, есть ли что курировать

```bash
ssh sarafan "cd ~/SETKA && ./venv/bin/python scripts/curate_pending.py --list --limit 20"
```
...
```bash
ssh sarafan "cd ~/SETKA && ./venv/bin/python scripts/curate_pending.py --apply" <<'JSON'
...
```bash
ssh sarafan "cd ~/SETKA && ./venv/bin/python scripts/curate_pending.py --stats"
```
