...
Локальной БД для разработки нет (см. [`docs/START_HERE.md`](../../docs/START_HERE.md)). Все SQL — только на проде. Если нужно «попробовать» — попроси пользователя запустить локальную PostgreSQL отдельно, или используй dry-run в транзакции на проде.
