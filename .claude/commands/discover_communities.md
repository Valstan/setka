...
- [`scripts/discover_scan.py`](../../scripts/discover_scan.py) — read-only VK-сканер.
  ...
  Флаги: `--per-label-top N` (ранжир ВНУТРИ темы), `--region-filter REGEX`
...
(из опыта kirov_obl):
- Общие новостные: `<город> новости`, `<город> онлайн`, `ЧП <город>`, `<область>`.
- Официальные по темам: `Министерство <X> <области>`, `Правительство`, `Губернатор`,
  вузы (`ВятГУ`), `Динамо <город>`, `Движение Первых <область>` и т.п.
- Темы (для области расширенный набор): `novost, proisshestviya, admin, kultura,
  sport, molodezh, nauka, promyshlennost, selhoz, zdorovie, zhkh, priroda`.
