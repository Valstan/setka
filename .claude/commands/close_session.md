...
Если PR — **только** про доки (изменены **только** `docs/SESSION_HANDOFF.md` и/или `docs/PENDING_FOLLOWUPS.md`), его можно авто-смёрджить после CI.

**Не делать авто-merge, если:**
- В PR есть **любые** файлы кроме `docs/SESSION_HANDOFF.md` / `docs/PENDING_FOLLOWUPS.md` (например, в ветке есть feature-коммиты с Шага 2 — это код, ему нужно ревью / `/reliz`).
