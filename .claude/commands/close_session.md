...
**Что будет, если НЕ вызвать и просто закрыть вкладку.** С 06.09 состояние пишут
хуки, без участия модели (`scripts/session_autosave.py`):

- после **каждого хода** — полная выжимка в `.claude/session-state/` (локально, под
  `.gitignore`), её печатает SessionStart новой сессии на этой же машине;
- после **каждого `gh pr merge` / `git push`** и при закрытии сессии — то же
  состояние без дословных реплик владельца уходит в ветку `session-state`, а
  незакоммиченное рабочее дерево — в `wip/<машина>` на GitHub.

Поэтому **факты не теряются ни на этой машине, ни на другой**: ветка, HEAD,
незакоммиченные файлы, номера PR, слова агента о ходе работы. Теряется то, чего
машина не знает: **почему** приняли решение, что отвергли и с какой причиной,
какой следующий шаг. Отсюда правило: **после мержа сессию можно закрывать молча**,
а `/close_session` вызывать тогда, когда нитку важно передать словами — в конце
дня, перед сменой компа, после закрытия milestone.
