...
```bash
ssh -o ConnectTimeout=10 sarafan "tail -n <N>x4 <path> | <grep_filter_if_any> | tail -n <N>" 2>&1
```
...
```bash
ssh -o ConnectTimeout=10 sarafan "journalctl -u <unit> --since '<since>' --no-pager -n <N> <grep_via_pipe>" 2>&1
```
