...
--
-- Идемпотентность (2026-05-22): убран дублирующийся блок (был копипастом ниже),
-- триггер обернут в `DROP IF EXISTS + CREATE` чтобы повторное применение не падало.
...
-- DROP TRIGGER IF EXISTS перед CREATE TRIGGER — потому что PG < 15
-- не поддерживает `CREATE TRIGGER IF NOT EXISTS`.
...
DROP TRIGGER IF EXISTS update_vk_tokens_updated_at ON vk_tokens;
