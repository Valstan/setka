...
--
-- Идемпотентность (2026-05-22): убран дублирующийся блок (был копипастом
-- внизу файла), все операции через `ADD COLUMN IF NOT EXISTS` /
-- `CREATE INDEX IF NOT EXISTS`. UPDATE идемпотентен по WHERE-условию.
...
-- Обновить существующие записи (только те, где validation_status NULL —
-- идемпотентно, повторный запуск не затронет уже-обработанные).
