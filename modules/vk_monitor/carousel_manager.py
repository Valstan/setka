...
class VKCarouselManager:
    ...
    async def get_next_region_to_scan(self, db: AsyncSession) -> Optional[RegionScanTask]:
        ...
        try:
            ...
            logger.info(
                f"Created scan task for region {oldest_region.code} "
                f"with {len(community_ids)} communities"
            )
            return task
    ...
    async def execute_region_scan(self, task: RegionScanTask, db: AsyncSession) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Starting scan for region {task.region_code} using token {task.token_name}"
            )
    ...
    async def _save_posts_to_db(
        self, posts_data: List[Dict], community: Community, db: AsyncSession
    ):
    ...
    async def get_carousel_status(self, db: AsyncSession) -> Dict:
        ...
        try:
            ...
            if last_scan:
                next_scan_time = last_scan.created_at + timedelta(
    ...
    async def optimize_scan_frequency(self, db: AsyncSession) -> Dict:
        ...
        try:
            ...
            if len(recent_scans) > 20:
                ...
                logger.info(
                    f"Increased scan interval to {self.scan_interval_minutes} minutes "
                    f"due to high load"
                )
                ...
                logger.info(
                    f"Decreased scan interval to {self.scan_interval_minutes} minutes "
                    f"due to low load"
                )
            ...
            return {
                ...
                "current_load": (
                    "high"
                    if len(recent_scans) > 20
                    else "low" if len(recent_scans) < 5 else "medium"
                ),
            }
