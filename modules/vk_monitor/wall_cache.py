...
* **Донорские стены — снимок стены целиком**, ``WALL_CACHE_TTL_SECONDS``
  (дефолт 300). Пять минут выбраны не «на глаз»: радар опрашивает источники раз
  в 10 минут, то есть ни один его проход не голодает, а счётчики поста, по
  которым считается рейтинг, стареют максимум на это окно.
