...
* **Своя ИНФО-стена района — РЕЗУЛЬТАТ, а не сырьё**, ``WALL_HISTORY_CACHE_TTL_SECONDS``
  (дефолт 21600, шесть часов). Из ста постов своей стены нужен один список коротких строк —
...
def wall_history_ttl_seconds() -> int:
    """TTL истории своей стены (список lip'ов источников). Дефолт 6 часов.

    **Почему так долго можно.** Запись сбрасывается явно при КАЖДОЙ публикации
    в эту стену (``invalidate_wall`` зовётся из ``publish_bulletin``), а между
    публикациями история по определению не меняется: новых сводок не выходило,
    значит и новых ссылок на источники в них не появилось. Протухнуть тут
    нечему, и TTL служит не свежести, а уборке ключей мёртвых районов.

    **Почему сначала стояло полчаса и почему это больше не нужно.** Полчаса
    выбирались, когда в кэше лежал СНИМОК стены — около мегабайта на район, то
    есть десятки мегабайт резидентной памяти на боксе, где свободно 274 МБ.
    После переделки на хранение результата запись весит ~2 КБ (замер на проде:
    2054 байта), все 57 районов — около 114 КБ. Ограничение осталось бы от
    причины, которой больше нет.

    **Что даёт длинный TTL.** Волна темы читает историю на каждом прогоне, а
    прогонов у района около двух десятков в сутки. При коротком TTL часть из
    них снова идёт в ВК просто потому, что ключ истёк между волнами — хотя
    публикаций (единственного события, меняющего историю) в этом промежутке не
    было. Шесть часов убирают этот лишний расход, не трогая свежесть.
    """
    try:
        return max(0, int(float(_getenv("WALL_HISTORY_CACHE_TTL_SECONDS", "21600"))))
    except ValueError:
        return 21600
