...
class PostClusterer:
    ...
    def __init__(self, time_window_hours: float = 6.0, min_cluster_size: int = 2):
        self.time_window_hours = time_window_hours
        self.min_cluster_size = min_cluster_size

    async def cluster_posts(
        self, posts: List[Any], by_category: bool = True, by_time: bool = True
    ) -> List[List[Any]]:
        """
    ...
    def _cluster_by_time(self, posts: List[Any]) -> List[List[Any]]:
        ...
        # Сортируем по дате
        sorted_posts = sorted(
            posts, key=lambda p: getattr(p, "date_published", None) or datetime.min, reverse=True
        )
    ...
    async def cluster_by_similarity(
        self, posts: List[Any], similarity_threshold: float = 0.7
    ) -> List[List[Any]]:
        """
