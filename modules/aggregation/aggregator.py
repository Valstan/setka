...
class NewsAggregator:
    ...
    async def aggregate(
        ...
        for post in posts[1:]:
            ...
            # Проверка лимитов
            if (
                current_text_length + post_text_length > self.max_text_length
                or current_media_count + post_media_count > self.max_media_items
            ):
                logger.debug("Reached limits, stopping aggregation")
                break
        ...
        categories = list(set(getattr(p, "ai_category", "novost") for p in [anchor] + additional))
    ...
    def _format_single_post(self, post: Any, title: str, hashtags: List[str] = None) -> str:
