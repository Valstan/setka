"""
Настройки конвейера дайджеста (парсинг → сборка поста).

Хранятся в RegionConfig.digest_filters (JSON):
{
  "defaults": { ... },
  "by_topic": {
...
def get_effective_pipeline_settings(region_config: Any, theme: str) -> Dict[str, Any]:
    """
    Сливает defaults → defaults из digest_filters → переопределение темы.
    Возвращает плоский словарь с числовыми ключами конвейера.
    """
    raw = getattr(region_config, "digest_filters", None)
    ...
    merged["max_posts_per_digest"] = max(1, min(merged["max_posts_per_digest"], 10))
...
def empty_digest_filters_template() -> Dict[str, Any]:
