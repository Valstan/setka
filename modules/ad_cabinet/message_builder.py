...
Здесь же — детектор испорченной кодировки (``looks_mangled``): шаблон,
у которого кириллица схлопнулась в вопросительные знаки, не должен уходить
рекламодателю (инцидент 2026-08-07, см. ``auto_greeting``).
"""
...
# Серия вопросительных знаков — след кириллицы, потерянной при записи текста
# через однобайтовую кодировку (каждая буква → '?'). В осмысленном шаблоне
# такой серии не бывает: даже «???» — уже подозрительно, а слово целиком
...
def looks_mangled(text: Optional[str]) -> bool:
    ...
    Признак — серия из трёх и более ``?`` подряд. Проверяется по **шаблону**,
    а не по отрендеренному тексту: подставляемые из БД имена приходят в
    нормальном UTF-8 и замаскировали бы поломку самого шаблона.
    """
    return bool(text) and bool(_MANGLED_RUN.search(text))
