...
async def prune_skipped(session, *, keep_days: int = 7) -> int:
    ...
    есть ``max_post_age_hours`` (дефолт сутки с 2026-09-22, до того 72 ч). Неделя —
    тот же срок с запасом; запас стал больше, и урезать его вслед за окном не нужно:
    у отдельных районов окно переопределяется своим значением.
