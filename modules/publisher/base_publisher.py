...
class BasePublisher(ABC):
    ...
    def format_post_text(
        self, post: Post, max_length: Optional[int] = None, add_source: bool = True
    ) -> str:
        """
    ...
    def extract_media(self, post: Post) -> Dict[str, list]:
        ...
        """
        result = {"photos": [], "videos": [], "documents": []}
        ...
        for attachment in post.attachments:
            ...
            if att_type == "photo":
                ...
                result["videos"].append(
                    {"owner_id": video.get("owner_id"), "video_id": video.get("id")}
                )

            elif att_type == "doc":
                doc = attachment.get("doc", {})
                result["documents"].append({"url": doc.get("url"), "title": doc.get("title")})

        return result

    def log_success(self, post_id: int, target: str, result: Any):
        """Log successful publishing"""
        self.logger.info(f"✅ Post {post_id} published to {self.platform_name}:{target} - {result}")
