...
⚠️ Грузить нужно **user-токеном админа** целевой группы: VK запрещает
``photos.getWallUploadServer``/``saveWallPhoto`` под community-auth — отдаёт
``[27] method is unavailable with group auth`` (probe 2026-06-19: 16/16 пабликов
сети заливаются user-токеном, 0/16 community-токеном). ``group_id`` всё равно
передаём в ``saveWallPhoto`` — владельцем фото становится **группа**, поэтому
attachment-строку нельзя переиспользовать на чужой стене (owner-mismatch → VK
дропнет вложение): грузим по каждой целевой группе отдельно. По одной картинке за
вызов upload-сервера; стена VK принимает максимум 10 вложений.
"""
