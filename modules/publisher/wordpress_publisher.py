...
class WordPressPublisher(BasePublisher):
    ...
    async def publish_post(self, post: Post, target_id: str = None, **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if media["photos"] and kwargs.get("auto_featured_image", True):
                featured_id = await self._upload_featured_image(media["photos"][0])
                if featured_id:
                    post_data["featured_media"] = featured_id

            # Create post via WordPress REST API
            response = requests.post(
                f"{self.api_url}/posts", json=post_data, auth=self.auth, timeout=30
            )
    ...
    async def update_post(self, wp_post_id: int, post: Post, **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = requests.post(
                f"{self.api_url}/posts/{wp_post_id}", json=post_data, auth=self.auth, timeout=30
            )
    ...
    async def delete_post(self, wp_post_id: int) -> bool:
        ...
        try:
            response = requests.delete(
                f"{self.api_url}/posts/{wp_post_id}", auth=self.auth, timeout=10
            )
            return response.status_code == 200
        except Exception as e:
