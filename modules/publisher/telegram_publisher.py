...
class TelegramPublisher(BasePublisher):
    ...
    async def publish_post(self, post: Post, target_id: str, **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            text = self._format_telegram_text(post, add_source=kwargs.get("add_source", True))
            ...
            if media["photos"]:
                result = await self._publish_with_photos(target_id, text, media["photos"], kwargs)
            else:
                result = await self._publish_text_only(target_id, text, kwargs)
    ...
    def _format_telegram_text(self, post: Post, add_source: bool = True) -> str:
        """
    ...
    async def _publish_text_only(self, chat_id: str, text: str, kwargs: dict):
    ...
    async def _publish_with_photos(
        self, chat_id: str, text: str, photo_urls: List[str], kwargs: dict
    ):
