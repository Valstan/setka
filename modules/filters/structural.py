...
class StructuralDuplicateFilter(DBFilter):
    ...
    async def apply(self, post: Any, context: dict) -> FilterResult:
        ...
        result = await session.execute(select(Post).where(Post.fingerprint_lip == lip))
...
class DateFilter(FastFilter):
    ...
    async def apply(self, post: Any, context: dict) -> FilterResult:
        ...
        if age_hours > self.max_age_hours:
            return FilterResult(
                passed=False, reason=f"Too old: {age_hours:.1f} hours (max {self.max_age_hours})"
            )
        ...
        return FilterResult(
            passed=True, score_modifier=score_modifier, metadata={"age_hours": age_hours}
        )
...
class BlacklistIDFilter(DBFilter):
    ...
    async def apply(self, post: Any, context: dict) -> FilterResult:
        ...
        if hasattr(post, "vk_owner_id"):
            ...
            if owner_id in black_ids:
                return FilterResult(passed=False, reason=f"Blacklisted owner ID: {owner_id}")
        ...
        if hasattr(post, "vk_from_id") and post.vk_from_id:
            ...
            if from_id in black_ids:
                return FilterResult(passed=False, reason=f"Blacklisted author ID: {from_id}")
    ...
    async def _get_black_ids(self, session: AsyncSession) -> set:
        ...
        # Проверить кэш
        if (
            self._black_ids_cache is not None
