            ...
            async with AsyncSessionLocal() as session:
                ...
                for i, community in enumerate(communities[:5], 1):  # Ограничиваем для теста
                    try:
                        ...
                        service_notifications.error(
                            f"Ошибка сканирования {community.name}: {str(e)}"
                        )

                # Уведомляем о завершении сбора постов
                processing_time = (datetime.now() - start_time).total_seconds()
                notify_post_collection_complete(
                    region.name, self.current_topic, total_posts, processing_time
                )
                ...
                if recent_posts:
                    ...
                    if approved_posts:
                        ...
                        service_notifications.post_select(
                            str(best_post.id),
    ...
    async def publish_post(self, post: Post, region: Region):
        ...
        try:
            ...
            if region.telegram_channel:
                logger.info(
                    f"Publishing post {post_id} to Telegram channel {region.telegram_channel}"
                )
