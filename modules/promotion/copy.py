...
# Темы, по которым человек ищет районный паблик. Взяты из живого описания
# Малмыж-ИНФО, которое владелец писал руками: это не догадка о поведении
# пользователя, а наблюдённый набор.
_SEARCH_TOPICS = (
    "новости",
    "происшествия",
    "афиша",
...
# Сколько населённых пунктов пускать в хвост. Их бывает 119 (Уржум) — весь
...
def render_search_tail(
    *,
    ...
    Зачем он вообще. 06.09 при попытке унифицировать описания выяснилось, что у
    старых групп под авторским текстом лежит список поисковых фраз («Новости
    Малмыжа», «Происшествия в Малмыже», «Расписание автобусов», «Подслушано
    Малмыж»…). Шаблон v3 их не содержал, поэтому «привести к единому виду»
    означало **потерять находимость** — ровно обратное цели ребрендинга.
    Прогон остановили на первой же группе и откатили.

    Отсюда правило: **унификация формы не должна отбирать функцию.** Прежде чем
    заменять авторский артефакт шаблонным, надо спросить, что этот артефакт
    делал; здесь он работал поисковым индексом.

    Населённые пункты — самая ценная часть хвоста (длинный хвост запросов), но
    они есть не у всех регионов и их бывает больше сотни, поэтому список
    необязателен и урезан до ``_MAX_LOCALITIES``.
    """
    name = (city or "").strip()
    if not name:
        return ""
    topics = ", ".join(_SEARCH_TOPICS)
    # Тире, а не «Новости {город}:». Название города у нас есть только в
    # именительном («Малмыж», «Тужа»), а фраза требует родительного — выходило
    # «Новости Малмыж», «Новости Тужа». Склонять русские топонимы кодом нельзя
    # без словаря: Малмыж→Малмыжа, Тужа→Тужи, Нема→Немы, Балтаси→Балтасей —
    # правила разные, а ошибка попадает в публичное описание. Формулировка
    # выбрана так, чтобы падеж вообще не требовался, и ключевые слова при этом
    # остались на месте. Родительный падеж района уже звучит строкой выше
    # («афиша Малмыжского района») — он берётся из готового заголовка сводок.
    lines = [f"{name} — {topics}."]

    # ``str(None)`` даёт непустое «None» — отсеивать надо ДО приведения к строке,
...
def render_group_description(
    *,
    ...
    Первые три строки — для человека, хвост — для поиска (``render_search_tail``).
    """
