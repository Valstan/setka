...
def plan_pairs(
    ...
    *,
    ...
    Returns:
        ``(pairs, orphans)`` — пары к публикации и районы, оставшиеся без промо,
        каждый со своей причиной. Второй список никогда не выбрасываем: он и есть
        честный ответ на вопрос «почему этот район ничего не получает».

    **Дневной лимит обрывает выдачу пар, но не осмотр.** Иначе список «без
    донора» молча укоротился бы до тех, кого успели посмотреть до исчерпания
    бюджета, и панель, сделанная ради честности, начала бы врать в самую
    выгодную сторону: одиннадцать недосягаемых районов исчезли бы из отчёта.
    """
    ...
    for target in ordered_targets:
        ...
        if len(pairs) >= max_pairs:
            # Донор есть, но сегодняшний лимит выбран. Это не «нет донора» —
            # и в отчёте это обязано читаться по-разному, иначе владелец решит,
            # что району помочь нечем, хотя ему просто не хватило очереди.
            orphans.append(
                OrphanTarget(
                    target=target,
                    reason="донор есть, но исчерпан дневной лимит действий",
                )
            )
            continue
