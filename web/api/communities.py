...
def extract_vk_id_from_input(vk_input: str, vk_api_instance) -> int:
    ...
    try:
        ...
        if result["type"] != "group":
            raise ValueError(
                f"Объект '{screen_name}' не является сообществом (найден тип: {result['type']})"
            )
...
class CommunityCreate(BaseModel):
    ...
    category: str = Field(
        ..., description="Категория: admin, novost, kultura, sport, reklama и т.д."
    )
...
@cache(ttl=300, key_prefix="communities")  # Cache for 5 minutes
async def get_region_communities(region_id: int, db: AsyncSession = Depends(get_db_session)):
...
async def add_community(
    community_data: CommunityCreate, db: AsyncSession = Depends(get_db_session)
):
    ...
    logger.info(
        f"Adding community: region_id={community_data.region_id}, vk_id={community_data.vk_id!r}, category={community_data.category}"
    )

    # Check if region exists
    region_result = await db.execute(select(Region).where(Region.id == community_data.region_id))
    ...
    try:
        ...
        existing = await db.execute(
            select(Community).where(
                Community.vk_id == vk_id, Community.region_id == community_data.region_id
            )
        )
...
async def update_community(
    community_id: int, community_data: CommunityUpdate, db: AsyncSession = Depends(get_db_session)
):
    """Update community"""
    from sqlalchemy.orm import selectinload
    ...
    if "region_id" in update_data:
        ...
        if new_region_id is not None and new_region_id != community.region_id:
            region_result = await db.execute(select(Region).where(Region.id == new_region_id))
...
async def delete_community(community_id: int, db: AsyncSession = Depends(get_db_session)):
    """Delete community"""
    ...
    await db.execute(delete(Community).where(Community.id == community_id))
