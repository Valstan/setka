...
Защита:
- авторизация — API-ключ на проект (БД ``gateway_keys`` — единый источник,
...
- агрегатный бюджет шлюза (env ``GATEWAY_GLOBAL_QUOTA_PER_MIN``) — сумма по
  всем потребителям держится ниже лимитов VK с запасом; превышение → 429
  потребителю, а НЕ проглатывание в счёт токена (мандат brain 2026-07-12);
...
from config.gateway import (
    ...
    gateway_disabled,
    get_gateway_global_quota_per_min,
...
async def enforce_quota(key_name: str = Depends(require_api_key)) -> str:
    ...
    # Слой 2 (мандат brain 2026-07-12): агрегатный бюджет шлюза — сумма по всем
    # потребителям держится ниже лимитов VK с запасом. Проверяется ПОСЛЕ
    # per-consumer квоты (чтобы долбящий потребитель получал свой 429, не выедая
    # общий бюджет) и ДО расхода токена: превышение → 429 потребителю, а не
    # проглатывание запроса в счёт VK-токена.
    global_per_min = get_gateway_global_quota_per_min()
    if global_per_min > 0:
        global_quota = GatewayQuota(_build_redis_client(), global_per_min, 0)
        allowed, retry_after = global_quota.check_and_consume("__gateway__", day=False)
        if not allowed:
            logger.info(
                "gateway: GLOBAL budget exceeded (by %s, retry_after=%ss)", key_name, retry_after
            )
            from modules.gateway.usage import record_request
            ...
            raise HTTPException(
                status_code=429,
                detail="Gateway budget exceeded",
                headers={"Retry-After": str(retry_after)},
            )
    return key_name
