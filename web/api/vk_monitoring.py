...
async def get_vk_stats(db: AsyncSession = Depends(get_db_session)):
    ...
    try:
        ...
        hourly_posts_result = await db.execute(
            select(func.count(Post.id)).where(Post.created_at >= hour_ago)
        )
        ...
        limit_usage = (
            min((requests_per_hour / 3600) / (active_tokens * 20) * 100, 100)
            if active_tokens > 0
            else 0
        )
        ...
        for token in tokens:
            logger.info(
                f"Processing token {token.name}: active={token.is_active}, status={token.validation_status}"
            )
            tokens_status.append(
                {
                    ...
                    "admin_groups": (
                        token.permissions.get("admin_groups", [])
                        if isinstance(token.permissions, dict)
                        else []
                    ),
                }
            )
    ...
    for name, token in VK_TOKENS.items():
        ...
        try:
            ...
            if user_info:
                results.append(
                    TokenValidationResponse(
                        token_name=name, is_valid=True, last_used=datetime.now(), error_message=None
                    )
                )
            else:
                results.append(
            ...
            results.append(
                TokenValidationResponse(
                    token_name=name, is_valid=False, last_used=None, error_message=str(e)
                )
            )
