...
async def add_schedule_item(
    schedule: ScheduleItem, session: AsyncSession = Depends(get_db_session)
):
...
async def update_schedule_item(
    schedule_id: int, schedule: ScheduleItem, session: AsyncSession = Depends(get_db_session)
):
    ...
    try:
        ...
        region_result = await session.execute(
            select(Region).where(Region.id == existing_schedule.region_id)
        )
            ...
            data=[
                {
                    ...
                    "last_run": (
                        existing_schedule.last_run.isoformat()
                        if existing_schedule.last_run
                        else None
                    ),
...
async def delete_schedule_item(schedule_id: int, session: AsyncSession = Depends(get_db_session)):
    ...
    try:
        ...
        return ScheduleResponse(success=True, data=[], message="Элемент расписания удалён")
