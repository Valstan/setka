...
⚠️ Привязка пишется ЦЕЛИКОМ: у существующего ключа передавайте ОБЕ половины.
Команда «правлю только имена» раньше молча стирала owner_id'ы (06.09 у
``VMALMYZHE`` так пропало ``[-158787639]``); теперь такой вызов отклоняется с
``partial_binding``. Очистить половину сознательно можно — передать флаг
пустым: ``--owner-ids=`` это ``[]``, а не «не передано».
    python scripts/issue_gateway_key.py VMALMYZHE --owner-ids "-158787639" \
        --screen-names "malmig_info,malmyzh_info43"

⚠️ Форма ``--owner-ids=<список>`` обязательна, когда список начинается с минуса:
argparse читает ``--owner-ids -217788511`` как ещё один флаг.
