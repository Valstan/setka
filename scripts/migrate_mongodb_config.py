...
async def migrate_region_config(mongo_db, session) -> int:
    ...
    for group_name, group_id in config_doc.get("all_my_groups", {}).items():
        ...
        result = await session.execute(Region.__table__.select().where(Region.code == region_code))
        ...
        if existing_config:
            ...
            existing_config.filter_group_by_region_words = config_doc.get(
                "filter_group_by_region_words", {}
            )
            existing_config.region_words = config_doc.get("kirov_words", {})
            existing_config.region_words = {
                **existing_config.region_words,
                **config_doc.get("tatar_words", {}),
            }
            existing_config.only_main_news = config_doc.get("only_main_news", [])
            existing_config.time_old_post = config_doc.get(
                "time_old_post", {"hard": 86400, "medium": 172800, "light": 604800}
            )
            existing_config.text_post_maxsize_simbols = config_doc.get(
            ...
            region_config = RegionConfig(
                ...
                region_words={
                    **config_doc.get("kirov_words", {}),
                    **config_doc.get("tatar_words", {}),
                },
                only_main_news=config_doc.get("only_main_news", []),
                time_old_post=config_doc.get(
                    "time_old_post", {"hard": 86400, "medium": 172800, "light": 604800}
                ),
            ...
            for theme_group_name, theme_group_id in theme_groups.items():
                ...
                result = await session.execute(
                    Community.__table__.select().where(
                        Community.vk_id == abs(theme_group_id), Community.category == theme
                    )
                )
...
async def migrate_filters(mongo_db, session) -> int:
    ...
    for pattern in blacklist:
        result = await session.execute(
            Filter.__table__.select().where(
                Filter.type == "blacklist_word", Filter.pattern == pattern
            )
        )
        existing = result.scalar_one_or_none()
        ...
        if not existing:
            f = Filter(
                ...
                action="delete",
                score_modifier=-100,
                description="Blacklisted word/phrase from MongoDB migration",
                is_active=True,
            )
            session.add(f)
            migrated += 1
    ...
    for region_word, words in kirov_words.items():
        for word in words:
            result = await session.execute(
                Filter.__table__.select().where(
                    Filter.type == "region_word", Filter.pattern == word
                )
            )
            existing = result.scalar_one_or_none()
