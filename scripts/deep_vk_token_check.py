...
for name in sorted(VK_TOKENS.keys()):
    ...
    try:
        ...
        perm_bits = {
            1: "notify", 2: "friends", 4: "photos", 8: "audio",
            16: "video", 32: "docs", 64: "notes", 128: "pages",
            2048: "wall", 4096: "messages", 8192: "email", 524288: "notify"
        }
    ...
    try:
        resp = api.wall.post(
            owner_id=-137760500,
            message=f"Диагностика токена {name} - авто-тест"
        )
        ...
        if 'Application is blocked' in err_str:
            ...
            print(f"  wall.post DENIED: Нет прав на запись в группу")
        else:
