...
def measure(
    ...
    **Почему выбор делается для КАЖДОГО сообщества, а не один раз на прогон.**
    Первая версия выбирала «постороннего» по первому району списка и дальше
    считала его посторонним везде. Первым по ``region_id`` идёт ``mi``, где
    двое доступных аккаунтов — админы, а третий подписчик: скрипт решил, что
    мерить нечем, и отказался измерять **все** сообщества вместо одного.
    Членство — свойство пары (аккаунт, сообщество), а не аккаунта.
    """
