...
**Форма поста** считается по числу элементов: до 2026-09-22 — по маркеру ``✍``, который
``modules/publisher/bulletin_builder.py`` ставил перед каждым элементом, после — по
ссылкам-атрибуциям (маркер снят решением владельца). Число элементов ограничено
...
def item_count(text: str) -> int:
    """Сколько элементов в теле поста. 0 — пост не нашей вёрстки.

    **Два признака, и это не перестраховка.** До 2026-09-22 каждый элемент
    начинался маркером ``✍``; в тот день маркер сняли решением владельца
    («чистые посты, как в оригинале»). Считать только по нему значило бы с этой
    даты возвращать 0 на КАЖДОМ нашем посте — то есть прибор тихо объявил бы
    всю новую вёрстку чужой и выбросил её из замера, ничем этого не выдав.
    Ровно тот класс отказа, ради разбора которого замер и писался.

    Поэтому: есть маркер — считаем по нему (исторические посты); нет — считаем
    ссылки-атрибуции, по одной на элемент, тем же разборщиком, которым волна
    вытаскивает источники из наших же сводок.

    Пост с ``hide_attribution`` не имеет ни маркера, ни ссылки и после 22.09
    неразличим с чужим — это известная граница прибора, а не случайность.
    """
    text = text or ""
    marked = text.count(POST_MARKER)
    if marked:
        return marked
    from utils.vk_wall_links import extract_wall_post_refs_from_text

    return len(extract_wall_post_refs_from_text(text))
