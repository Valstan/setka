...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        result = await session.execute(select(Region).where(Region.code == "test"))
        ...
        result = await session.execute(
            select(WorkTable).where(WorkTable.region_code == "test", WorkTable.theme == "novost")
        )
        ...
        if split_result["mourning_markers"]:
            logger.info("Найдены mourning-маркеры:")
