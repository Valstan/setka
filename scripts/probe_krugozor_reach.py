...
сводки посчитать охват → «при хорошем охвате +обед 13:00». Запланировано
«через ~неделю», на 2026-06-24 прошло 10 дней — пора снять точку.

Поток «Кругозор» (modules/krugozor_broadcast.py) публикует научпоп-сводка нативным
...
стенах: ``wall.get`` по каждому региону, фильтруем по заголовку сводки, читаем
...
постов (не-сводка, не-закреп) в том же окне. «Хороший охват» = сводка читают
...
# заголовку: ловит и старый «🔭 Научпоп-сводка», и новый «🔭 Научпоп-сводка» (рефактор
...
def _recommend(rows: List[Dict[str, Any]], pooled_bulletin: List[int]) -> Dict[str, Any]:
    ...
    **Метрика — медиана per-region отношений, НЕ pooled.** Pooled-сравнение
    (медиана всех сводок / медиана всех локальных) даёт парадокс Симпсона: пул
    ...
    Эвристика порогов (озвучить владельцу, не догма): сводка уверенно «читают», если
    ...
    if not pooled_bulletin:
        ...
        why = (
            f"типичный регион даёт сводке {typ}% просмотров локального поста "
            "(≥50%) — 2× оправдан"
        )
    elif typ < 25:
        verdict = "KEEP_1X"
